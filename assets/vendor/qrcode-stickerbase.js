/* StickerBase QR encoder · derived from Kazuhiko Arase QRCode for JavaScript (MIT). */
(function(g){
...
__mods['QRCode']=function(module,exports,__req){
//---------------------------------------------------------------------
...
function __req(name){ if(__cache[name]) return __cache[name].exports; const fn=__mods[name]; if(!fn) throw new Error("QR module not found: "+name); const module={exports:{}}; __cache[name]=module; fn(module,module.exports,__req); return module.exports; }
const QRCode=__req("QRCode"), Level=__req("QRErrorCorrectLevel");
g.StickerBaseQRCode={matrix:function(text,level){const qr=new QRCode(-1,Level[level||"M"]||Level.M);qr.addData(String(text));qr.make();return qr.modules.map(r=>r.slice());},moduleCount:function(text,level){return this.matrix(text,level).length;}};
})(globalThis);