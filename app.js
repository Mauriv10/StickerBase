const APP_VERSION=globalThis.WC26_CONFIG?.version||"704.12.16";
...
function renderTradeDetectedItems(items){
 const type=inferCollectionType(projects?.[activeProjectId]);
 if(!items.length)return "";
 return `<div class="trade-detected-list">${items.map(item=>{
  const name=type==="world-cup-2026"?`${item.team} ${item.displayCode}`:clubTradeNameForItem(type,item);
  const status=item.available>0?`<span class="trade-detected-stock available">Puedes ofrecer ${item.available}${item.requestedUnits>1?` de ${item.requestedUnits} pedidas`:""}</span>`:`<span class="trade-detected-stock unavailable">Sin unidades para ofrecer</span>`;
  return `<article class="trade-detected-item"><div class="trade-detected-icon">${flagHTML(item.team)}</div><div class="trade-detected-copy"><strong>${escapeTradeHtml(name)}</strong><small>${escapeTradeHtml(item.team)} · ${escapeTradeHtml(item.displayCode)}</small>${status}</div><div class="trade-detected-owned">x${item.owned}</div></article>`;
 }).join("")}</div>`;
}
function renderTradeAnalyzerResult(){
 ...
 const target=getTarget(),analysed=parsed.found.map(item=>{const owned=Number(inventory?.[item.team]?.[item.internalCode])||0;const stockAvailable=Math.max(0,owned-target);const requestedUnits=Math.max(1,Number(item.requestedUnits)||1);return {...item,owned,stockAvailable,requestedUnits,available:Math.min(stockAvailable,requestedUnits)};}),available=analysed.filter(x=>x.available>0),safeAvailable=available.filter(x=>!isTradeStar(x)&&!isTradeProtected(x)),previewItems=safeAvailable.map(item=>({...item,selectedUnits:item.available})),previewUnits=previewItems.reduce((sum,item)=>sum+(item.selectedUnits||0),0),withoutStock=analysed.filter(x=>x.available<=0).length;
 result.innerHTML=`<button id="editTradeAnalyzerList" class="trade-back-button" type="button">← Editar lista</button><div class="trade-clean-card"><div class="trade-clean-status"><strong>${analysed.length} cromos detectados</strong>${withoutStock?`<span>${withoutStock} sin unidades disponibles</span>`:""}${parsed.invalid.length?`<span>${parsed.invalid.length} ${parsed.invalid.length===1?"línea no entendida":"líneas no entendidas"}</span>`:""}</div>${renderTradeDetectedItems(analysed)}${parsed.invalid.length?`<div class="trade-inline-alert">${renderInvalidLines(parsed.invalid)}</div>`:""}${safeAvailable.length?`<details class="trade-offer-preview" open><summary>Lo que puedes ofrecer · ${previewUnits}</summary><div class="balanced-sticker-list">${renderBalancedStickerList(previewItems)}</div></details>`:`<div class="trade-no-offer">Los cromos se han reconocido, pero ahora mismo no tienes unidades disponibles para ofrecer.</div>`}<button id="copyTradeAnalyzerAll" class="primary trade-main-action" type="button" ${safeAvailable.length?"":"disabled"}>${safeAvailable.length?"Copiar lo que puedes ofrecer":"Nada disponible para copiar"}</button><button id="prepareBalancedTrade" class="secondary trade-main-action" type="button" ${safeAvailable.length?"":"disabled"}>${safeAvailable.length?"Preparar intercambio":"Sin stock para preparar"}</button></div>`;
 ...
 $("#copyTradeAnalyzerAll")?.addEventListener("click",async event=>{if(!safeAvailable.length)return;const fb=actionFeedback(event.currentTarget,{busy:"Copiando…",done:"Copiado ✓"});try{await copyShareText(tradeCopyLines(safeAvailable).join("\n"));fb.success();}catch{fb.fail("Error");}});
 $("#prepareBalancedTrade")?.addEventListener("click",()=>{if(safeAvailable.length)renderExchangeStep(safeAvailable);});
}
