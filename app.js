const APP_VERSION=globalThis.WC26_CONFIG?.version||"704.14.54";
...
function pokemonSingleOwnedCardHtml(card){
 const incoming=card.status==="incoming";return `<article class="pokemon-single-owned ${incoming?"is-incoming":""}" data-single-id="${collectionSafeText(card.id)}"><div class="pokemon-single-owned-image ${card.image?"is-clickable":""}" ${card.image?`data-owned-single-preview="${collectionSafeText(card.id)}" role="button" tabindex="0" aria-label="Ver ${collectionSafeText(card.name||"carta Pokémon")} en grande"`:""}>${card.image?`<img src="${collectionSafeText(card.image)}" alt="${collectionSafeText(card.name||"Carta Pokémon")}" loading="lazy">`:'<span>PK</span>'}</div><div class="pokemon-single-owned-copy"><small>${collectionSafeText(card.setName||"Pokémon TCG")}</small><strong>${collectionSafeText(card.name||"Carta Pokémon")}</strong><span>#${collectionSafeText(card.number||"—")} · ${pokemonSinglesLanguageLabel(card.language)}${card.rarity?` · ${collectionSafeText(card.rarity)}`:""}</span></div><div class="pokemon-single-owned-actions"><button type="button" data-single-incoming="${collectionSafeText(card.id)}" class="${incoming?"active":""}">${incoming?"📦 En camino":"＋ En camino"}</button><button type="button" data-single-remove="${collectionSafeText(card.id)}">Eliminar</button></div></article>`;
}
