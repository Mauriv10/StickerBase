const APP_VERSION=globalThis.WC26_CONFIG?.version||"704.14.53";
...
function renderPokemonIncomingView(){
 ...
 // En iOS/PWA no fiarse solo del tipo inferido: mientras cambia la pestaña el chrome Pokémon
 // puede estar ya activo aunque el proyecto aún esté terminando de renderizar.
 ...
 const tradeView=$("#tradeView"),pokeView=$("#pokemonIncomingView"),panini=$("#paniniTradeView");
 ...
 // Controlar también el contenedor padre. Safari/PWA puede conservar display:none del estado anterior.
 if(tradeView&&mainTab==="trade"){tradeView.hidden=false;tradeView.removeAttribute("hidden");tradeView.style.setProperty("display","block","important");}
 document.body.classList.toggle("pokemon-incoming-active",Boolean(isPokemon&&mainTab==="trade"));
 if(pokeView){if(isPokemon){pokeView.hidden=false;pokeView.removeAttribute("hidden");}else pokeView.hidden=true;pokeView.style.setProperty("display",isPokemon?"block":"none","important");}
 if(panini){if(isPokemon)panini.setAttribute("hidden","");else panini.removeAttribute("hidden");panini.hidden=isPokemon;panini.style.setProperty("display",isPokemon?"none":"block","important");}
 if(!isPokemon)return;
...
function renderPokemonSinglesCollection(){
 ...
 list.innerHTML=`<section class="pokemon-singles-shell"><header class="pokemon-singles-hero"><div><span>POKÉMON · COLECCIÓN LIBRE</span><h2>Mis Singles</h2><p>Guarda aquí cualquier carta que te guste, aunque no completes su expansión.</p></div><div class="pokemon-singles-kpis"><b>${project.pokemonSingles.filter(c=>c.status!=="incoming").length}</b><span>en colección</span><b>${project.pokemonSingles.filter(c=>c.status==="incoming").length}</b><span>en camino</span></div></header><section class="pokemon-single-add"><div class="pokemon-single-add-head"><div><strong>Añadir carta</strong><small>Busca por nombre o número en todo Pokémon TCG</small></div></div><div class="pokemon-single-search-row"><select id="pokemonSingleLanguage" aria-label="Idioma"><option value="es">Castellano</option><option value="en">English</option><option value="ja">日本語</option><option value="zh-cn">中文</option></select><div class="pokemon-single-query-wrap"><span>⌕</span><input id="pokemonSingleQuery" autocomplete="off" autocapitalize="off" spellcheck="false" inputmode="search" enterkeyhint="search" placeholder="Busca Pokémon o número…"></div><button id="pokemonSingleSearchButton" type="button">Buscar</button></div><div id="pokemonSingleSearchStatus" class="pokemon-single-search-status"></div><div id="pokemonSingleSearchResults" class="pokemon-single-search-results"></div></section><section class="pokemon-singles-list"><div class="pokemon-singles-list-head"><strong>Cartas registradas</strong><span>${cards.length}</span></div>${cards.length?cards.map(card=>pokemonSingleOwnedCardHtml(card)).join(""):'<div class="pokemon-singles-empty">Todavía no has registrado singles. Busca arriba la primera carta que quieras guardar.</div>'}</section></section>`;
 const query=$("#pokemonSingleQuery");if(query&&q)query.value=teamSearch.value||"";$("#pokemonSingleSearchButton")?.addEventListener("click",()=>pokemonSinglesSearch(true));query?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();pokemonSinglesSearch(true)}});query?.addEventListener("input",()=>{clearTimeout(pokemonSinglesSearchTimer);const value=query.value.trim();if(value.length<2){pokemonSinglesSearchResults=[];const status=$("#pokemonSingleSearchStatus");if(status)status.textContent=value?"Escribe al menos 2 caracteres.":"";renderPokemonSinglesSearchResults();return;}pokemonSinglesSearchTimer=setTimeout(()=>pokemonSinglesSearch(false),350);});$("#pokemonSingleLanguage")?.addEventListener("change",()=>{if((query?.value||"").trim().length>=2)pokemonSinglesSearch(true);});renderPokemonSinglesSearchResults();wirePokemonSingleCards();
}
...
function renderPokemonSinglesSearchResults(){
 const root=$("#pokemonSingleSearchResults");if(!root)return;const project=pokemonSinglesProject();const existing=new Set((project?.pokemonSingles||[]).map(card=>card.id));
 root.innerHTML=pokemonSinglesSearchResults.slice(0,24).map((card,index)=>{const target=pokemonSinglesCollectionTarget(card),canonicalImage=target?(target.meta?.images?.small||pokemonDirectImageUrl(target.type,target.code,"small")||""):"",displayImage=canonicalImage||card.image||pokemonSinglesFallbackImage(card),variant=target?.options?.length?`<select class="pokemon-single-route-variant" data-single-variant="${collectionSafeText(card.id)}">${target.options.map(([key,label])=>`<option value="${collectionSafeText(key)}">${collectionSafeText(label)}</option>`).join("")}</select>`:"",destination=target?`<em class="pokemon-single-destination">→ ${collectionSafeText(target.project.name)}</em>`:`<em class="pokemon-single-destination">→ Mis Singles</em>`;card.displayImage=displayImage;return `<article class="pokemon-single-result" data-single-result-index="${index}"><div class="pokemon-single-search-image ${displayImage?"is-clickable":""}" ${displayImage?`data-single-preview="${index}" role="button" tabindex="0" aria-label="Ver ${collectionSafeText(card.name)} en grande"`:""}>${displayImage?`<img src="${collectionSafeText(displayImage)}" alt="${collectionSafeText(card.name)}" loading="lazy">`:'<span>PK</span>'}</div><section><small>${collectionSafeText(card.setName)}</small><strong>${collectionSafeText(card.name)}</strong><span>#${collectionSafeText(card.number)}${card.rarity?` · ${collectionSafeText(card.rarity)}`:""}</span>${destination}${variant}<em class="pokemon-single-existing-state" data-single-existing-state hidden></em></section><aside><button type="button" data-add-single="${index}" data-single-status="owned">＋ La tengo</button><button type="button" data-add-single="${index}" data-single-status="incoming">📦 En camino</button></aside></article>`}).join("");
 root.querySelectorAll("[data-add-single]").forEach(button=>button.onclick=()=>pokemonAddSingle(Number(button.dataset.addSingle),button.dataset.singleStatus||"owned"));
 root.querySelectorAll("[data-single-result-index]").forEach(article=>pokemonSinglesRefreshResultState(article,Number(article.dataset.singleResultIndex)));
 root.querySelectorAll("[data-single-variant]").forEach(select=>select.addEventListener("change",()=>{const article=select.closest("[data-single-result-index]");if(article)pokemonSinglesRefreshResultState(article,Number(article.dataset.singleResultIndex));}));
 root.querySelectorAll("[data-single-preview]").forEach(el=>{const open=()=>{const card=pokemonSinglesSearchResults[Number(el.dataset.singlePreview)];if(card?.displayImage||card?.image)openPokemonCardImageViewer(card.displayImage||card.image,card.number,{name:card.name});};el.onclick=open;el.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();open();}};});
}
function pokemonAddSingle(index,status="owned"){const project=pokemonSinglesProject(),card=pokemonSinglesSearchResults[index];if(!project||!card)return;const target=pokemonSinglesCollectionTarget(card);if(target){const state=pokemonSinglesTargetState(card,target);if(status==="owned"&&state.owned){showToast(`✓ Ya tienes esta carta en ${target.project.name}`);return;}if(status==="incoming"&&state.incoming){showToast(`📦 Esta carta ya está En camino en ${target.project.name}`);return;}pokemonSinglesAddToCollection(card,status,target);return;}project.pokemonSingles=project.pokemonSingles||[];const existing=project.pokemonSingles.find(item=>item.id===card.id);if(existing){showToast(existing.status==="incoming"?"📦 Esta carta ya está En camino":"✓ Esta carta ya está en Mis Singles");return;}project.pokemonSingles.push({...card,status:status==="incoming"?"incoming":"owned",addedAt:new Date().toISOString()});persistProjects();scheduleCloudSave();renderPokemonSinglesCollection();updateNavigationBadges();showToast(status==="incoming"?"📦 Single añadido a En camino":"✓ Single añadido");}
