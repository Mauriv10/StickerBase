const APP_VERSION=globalThis.WC26_CONFIG?.version||"704.14.74";
...
const COLLECTION_DEFINITIONS={
  ...
  "pokemon-singles":{label:"POKÉMON · MIS SINGLES",subtitle:"Cartas sueltas de cualquier expansión",icon:"★",theme:"pokemon"}
};
...
const POKEMON_SET_DEFS={
 ...
 "pokemon-singles":{name:"Mis Singles",seedType:"pokemon-singles",official:0,total:0,ranges:[],virtual:true}
};
...
function pokemonInventoryTemplate(type){const def=POKEMON_SET_DEFS[type];if(!def)return {};return Object.fromEntries(def.ranges.map(([section,start,end])=>[section,Object.fromEntries(Array.from({length:end-start+1},(_,i)=>[pokemonCode(start+i),0]))]));}
...
const FIRST_PARTNER_IMAGE_FALLBACKS={"046":"https://unownarchives.b-cdn.net/uploads/2026/05/886615.png","047":"https://unownarchives.b-cdn.net/uploads/2026/05/886616.png","048":"https://unownarchives.b-cdn.net/uploads/2026/05/886617.png","049":"https://unownarchives.b-cdn.net/uploads/2026/05/886618.png","050":"https://unownarchives.b-cdn.net/uploads/2026/05/886619.png","051":"https://unownarchives.b-cdn.net/uploads/2026/05/886620.png","052":"https://unownarchives.b-cdn.net/uploads/2026/05/886621.png","053":"https://unownarchives.b-cdn.net/uploads/2026/05/886622.png","054":"https://unownarchives.b-cdn.net/uploads/2026/05/886623.png","055":"https://unownarchives.b-cdn.net/uploads/2026/07/891870.png","056":"https://unownarchives.b-cdn.net/uploads/2026/07/891880.png","057":"https://unownarchives.b-cdn.net/uploads/2026/07/891888.png","058":"https://unownarchives.b-cdn.net/uploads/2026/07/891889.png","059":"https://unownarchives.b-cdn.net/uploads/2026/07/891890.png","060":"https://unownarchives.b-cdn.net/uploads/2026/07/891891.png","061":"https://unownarchives.b-cdn.net/uploads/2026/07/891892.png","062":"https://unownarchives.b-cdn.net/uploads/2026/07/891893.png","063":"https://unownarchives.b-cdn.net/uploads/2026/07/891894.png"};
function pokemonDirectImageUrl(type,code,size="small"){const def=POKEMON_SET_DEFS[type];if(!def?.setId)return "";if(def.imageProvider==="tcgdex-mep"){const key=pokemonCode(Number(code));if(FIRST_PARTNER_IMAGE_FALLBACKS[key])return FIRST_PARTNER_IMAGE_FALLBACKS[key];const quality=size==="large"?"high":"low";return `https://assets.tcgdex.net/en/me/mep/${key}/${quality}.webp`;}return `https://images.scrydex.com/pokemon/${def.setId}-${Number(code)}/${size}`;}
...
function collectionInventoryTemplate(type){
 ...
 if(isPokemonCollectionType(type))return pokemonInventoryTemplate(type);
 return {};
}
function seedTypeForCollection(type){
 ...
 if(isPokemonCollectionType(type))return POKEMON_SET_DEFS[type].seedType;
...
function inferCollectionType(projectOrSeed={}){
 ...
 for(const type of Object.keys(POKEMON_SET_DEFS)){const def=POKEMON_SET_DEFS[type];if(seed===def.seedType||name.includes(def.name.toLowerCase()))return type;}
...
function syncCollectionChrome(collectionType){
 ...
 const detached=collectionType==="liga-este-2026-27"||collectionType==="megacracks-2026-27"||isPokemonCollectionType(collectionType);
 if(detached){
...
function syncPokemonHeaderFace(project=projects?.[activeProjectId]){
 ...
 if(type==="pokemon-singles"){face.hidden=true;return;}
 face.hidden=false;
 if(img){
...
function applyCollectionIdentity(project=projects?.[activeProjectId]){
 ...
 const teamLabel=document.querySelector("#teamSelectorLabel");if(teamLabel)teamLabel.textContent=isPokemonCollectionType(project.collectionType)?"Rareza":(project.collectionType==="liga-este-2026-27"||project.collectionType==="megacracks-2026-27")?"Club":"Selección";
 ...
 const logo=document.querySelector("#ligaEsteHeaderLogo");if(logo){logo.hidden=project.collectionType==="world-cup-2026"||isPokemonCollectionType(project.collectionType);logo.textContent=project.collectionType==="megacracks-2026-27"?"Mega\nCracks":"LIGA\nESTE";}
 syncPaniniPremiumHeader(project);
 syncPokemonHeaderFace(project);
}
...
function stickerDisplayLabel(team,code){
 ...
 if(isPokemonCollectionType(activeType)){const meta=pokemonCardMeta(code);return meta?`#${String(code)} · ${meta.name}`:`#${String(code)}`;}
...
function ensureProjectInventorySchema(project){
 ...
 if(isPokemonCollectionType(project.collectionType)){
   const template=pokemonInventoryTemplate(project.collectionType);
   for(const [section,codes] of Object.entries(template)){if(!project.inventory[section])project.inventory[section]={};for(const code of Object.keys(codes))if(!Object.prototype.hasOwnProperty.call(project.inventory[section],code))project.inventory[section][code]=0;}
   return;
 }
...
function bootstrapProjectsFromSeed(seedData){
 ...
 Object.keys(POKEMON_SET_DEFS).forEach(type=>masterInventories[POKEMON_SET_DEFS[type].seedType]=pokemonInventoryTemplate(type));
...
function ensurePokemonProjects(){
 ...
 for(const [type,def] of Object.entries(POKEMON_SET_DEFS)){
   ...
   if(Object.values(projects||{}).some(p=>inferCollectionType(p)===type))continue;
   const p=defaultProject(def.name,1,pokemonInventoryTemplate(type),def.seedType);
   p.collectionType=type;p.pokemonMeta={};p.pokemonDataSource=def.source;p.teamOrder=Object.keys(p.inventory);
   projects[p.id]=p;changed=true;
 }
 localStorage.setItem(POKEMON_SEED_MIGRATION_KEY,"1");
...
function comparableProjects(source=projects){
 ...
 return Object.fromEntries(Object.entries(normalized).map(([id,p])=>[id,{
   ...
   inventory:p.inventory||{},pokemonVariants:p.pokemonVariants||{},pokemonIncoming:p.pokemonIncoming||{},pokemonSingles:p.pokemonSingles||[],collectionOptions:p.collectionOptions||normalizedCollectionOptionsForComparison(),
...
function collectionAllTeamsLabel(){const type=inferCollectionType(projects?.[activeProjectId]);return type==="world-cup-2026"?"Todas las selecciones":isPokemonCollectionType(type)?"Todas las rarezas":"Todos los clubes";}
...
function changeQuantity(team,code,delta,button){
 ...
 if(delta<0&&next===0){const active=projects?.[activeProjectId];if(active&&isPokemonCollectionType(inferCollectionType(active))&&inferCollectionType(active)!=="pokemon-singles")pokemonSinglesRemoveAlbumReferenceEverywhere(active,team,code);}
...
function collectionStickerMatches(team,code,qty){
 ...
 if((type==="liga-este-2026-27"||type==="megacracks-2026-27")&&isPendingCollectionItem(team,code))return effectiveFilter==="all";
...
function renderPokemonDashboard(){
 ...
 const active=Boolean(def);wrap.hidden=!active||Boolean(def?.virtual);document.body.classList.toggle("pokemon-ui-active",active);if(!active||def?.virtual)return;
...
function openPokemonCardImageViewer(image,code,meta={}){
 closePokemonCardViewer();
 const small=String(image||meta.images?.small||"");
 const large=String(meta.images?.large||small).replace(/\/low\.webp(?:\?.*)?$/i,"/high.webp");
 if(!large)return;
...
function pokemonCardRow(section,code,qty){
 ...
 const incomingVariant=(section==="BASE"&&!ex&&pokemonBaseVariantOptions(p,code,meta).length)?pokemonSelectedVariant(p,code):"fixed";
 const incoming=pokemonIncomingHas(p,section,code,incomingVariant);
 row.innerHTML=`<div class="pokemon-number">${collectionSafeText(code)}</div><div class="pokemon-thumb">${img?`<img src="${collectionSafeText(img)}" data-fallback="${collectionSafeText(fallback)}" alt="${collectionSafeText(meta.name||`Carta ${code}`)}" loading="lazy" decoding="async">`:"<span>PK</span>"}</div><div class="pokemon-card-main"><strong>${collectionSafeText(meta.name||`#${code}`)}</strong>${controls}<button type="button" class="pokemon-incoming-toggle ${incoming?"active":""}" aria-pressed="${incoming?"true":"false"}">${incoming?"📦 En camino":"＋ En camino"}</button></div>`;
 ...
 row.querySelector(".pokemon-incoming-toggle")?.addEventListener("click",()=>pokemonToggleIncomingForCard(p,section,code,incomingVariant));
...
function renderPokemonCollection(){
 const list=$("#globalCollectionList"),p=projects?.[activeProjectId],def=POKEMON_SET_DEFS[inferCollectionType(p)];if(!list||!p||!def)return;if(def.virtual){renderPokemonSinglesCollection();return;}renderPokemonDashboard();list.innerHTML="";
 ...
 if(!list.children.length)list.innerHTML='<div class="collection-empty">No hay cartas para este filtro.</div>';
}
...
function calculateProjectStatistics(){
 ...
 let normalMissing=0,normalComplete=0,baseOwned=0,baseAvailable=0;
...
function renderPokemonStatistics(){
 ...
 if(inferCollectionType(active)==="pokemon-singles"){renderPokemonSinglesStatistics(active);return;}
...
const POKEMON_TCGDEX_BETA_API="https://beta.api.tcgdex.net/v2";
const POKEMON_TCG_API="https://api.pokemontcg.io/v2";
const CARDMARKET_POKEMON_PRODUCTS_URL="https://downloads.s3.cardmarket.com/productCatalog/productList/products_singles_6.json";
const CARDMARKET_POKEMON_PRICES_URL="https://downloads.s3.cardmarket.com/productCatalog/priceGuide/price_guide_6.json";
const pokemonSinglesPtcgCache=new Map();
let pokemonSinglesCardmarketDataPromise=null;
let pokemonSinglesCardmarketDataLoadedAt=0;
let pokemonSinglesCardmarketRefreshPromise=null;
let pokemonSinglesCardmarketCacheReadPromise=null;
let pokemonSinglesSearchPriceHydrationToken=0;
let pokemonSinglesStoredPriceRefreshInFlight=false;
const POKEMON_CARDMARKET_IDB_NAME="stickerbase.cardmarket.v1";
const POKEMON_CARDMARKET_IDB_STORE="snapshots";
const POKEMON_CARDMARKET_IDB_KEY="pokemon-singles";
const POKEMON_CARDMARKET_FETCH_TIMEOUT_MS=60000;
const POKEMON_CARDMARKET_REFRESH_MS=24*60*60*1000;
const pokemonSinglesCardmarketMatchCache=new Map();
let pokemonSinglesSearchResults=[],pokemonSinglesSearchBusy=false,pokemonSinglesSearchQueued=false,pokemonSinglesSearchTimer=null;
let pokemonSinglesScanBusy=false,pokemonSinglesScanHint=null,pokemonSinglesTesseractPromise=null,pokemonSinglesTesseractWorker=null;
const POKEMON_SINGLES_TESSERACT_URL="https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js";
...
function pokemonSinglesRecentRead(key){try{return JSON.parse(localStorage.getItem(key)||"[]")||[]}catch{return []}}
function pokemonSinglesRecentPush(key,card){if(!card)return;const clean={id:card.id,tcgdexId:card.tcgdexId,name:card.name,number:card.number,setName:card.setName,setId:card.setId,rarity:card.rarity,language:card.language,image:card.displayImage||card.image||""};let rows=pokemonSinglesRecentRead(key).filter(x=>x.id!==clean.id);rows.unshift(clean);rows=rows.slice(0,20);try{localStorage.setItem(key,JSON.stringify(rows))}catch{}}
function pokemonSinglesRecentCardHtml(card,kind,index){const image=card.image||pokemonSinglesFallbackImage(card);return `<button type="button" class="pokemon-discover-recent-card" data-recent-kind="${kind}" data-recent-index="${index}">${image?`<img src="${collectionSafeText(image)}" alt="${collectionSafeText(card.name||"Carta Pokémon")}" loading="lazy">`:'<span>PK</span>'}<strong>${collectionSafeText(card.name||"Carta Pokémon")}</strong><small>${collectionSafeText(card.setName||"")}${card.number?` · #${collectionSafeText(card.number)}`:""}</small></button>`;}
function pokemonSinglesRecentStrip(title,key,kind){const rows=pokemonSinglesRecentRead(key);if(!rows.length)return "";return `<section class="pokemon-discover-recent"><div class="pokemon-discover-section-head"><span>${collectionSafeText(title)}</span></div><div class="pokemon-discover-recent-strip">${rows.slice(0,10).map((c,i)=>pokemonSinglesRecentCardHtml(c,kind,i)).join("")}</div></section>`;}
const pokemonSinglesImageHydration=new Set();
const pokemonSinglesImageProbeCache=new Map();
function pokemonSinglesProbeImage(url){
 url=String(url||"").trim();if(!url)return Promise.resolve(false);if(pokemonSinglesImageProbeCache.has(url))return pokemonSinglesImageProbeCache.get(url);
 const promise=new Promise(resolve=>{if(typeof Image==="undefined"){resolve(true);return;}const img=new Image();let done=false,timer=null;const finish=ok=>{if(done)return;done=true;if(timer)clearTimeout(timer);img.onload=null;img.onerror=null;resolve(Boolean(ok));};img.onload=()=>finish((img.naturalWidth||0)>20&&(img.naturalHeight||0)>20);img.onerror=()=>finish(false);timer=setTimeout(()=>finish(false),8000);img.src=url;});pokemonSinglesImageProbeCache.set(url,promise);return promise;
}
function pokemonSinglesBrokenImageUrls(card){const rows=Array.isArray(card?.imageFailedUrls)?card.imageFailedUrls:[];return new Set(rows.filter(Boolean));}
function pokemonSinglesRememberBrokenImage(card,url){url=String(url||"").trim();if(!card||!url)return;const rows=Array.isArray(card.imageFailedUrls)?card.imageFailedUrls.filter(Boolean):[];if(!rows.includes(url))rows.push(url);card.imageFailedUrls=rows.slice(-8);card.imageBrokenAt=new Date().toISOString();}
function pokemonSinglesKnownAlbumImage(card,size="small"){
 const target=pokemonSinglesTargetFromMirror(card)||pokemonSinglesCollectionTarget(card);if(!target)return "";const meta=target.meta||{},fromMeta=size==="large"?meta.images?.large:meta.images?.small;if(fromMeta)return fromMeta;return pokemonDirectImageUrl(target.type,target.code,size)||"";
}
function pokemonSinglesOwnedImageError(img){
 try{img.style.display="none";img.parentElement?.classList.add("image-missing");const article=img.closest?.("[data-single-id]"),project=pokemonSinglesProject(),card=project?.pokemonSingles?.find(item=>item.id===article?.dataset?.singleId);if(!project||!card)return;pokemonSinglesRememberBrokenImage(card,img.currentSrc||img.src||card.image);pokemonSinglesEnsureCardImage(project,card,()=>{if(project.id===activeProjectId&&mainTab==="collection")renderPokemonSinglesCollection();},true);}catch{}
}
function pokemonSinglesIncomingImageError(img){
 try{img.style.display="none";img.parentElement?.classList.add("image-missing");const key=img.dataset?.incomingImageKey,row=pokemonIncomingEntries().find(entry=>entry.key===key);if(row?.kind!=="single"||!row?.card)return;pokemonSinglesRememberBrokenImage(row.card,img.currentSrc||img.src||row.card.image);pokemonSinglesEnsureCardImage(row.project,row.card,()=>{if(mainTab==="trade"||pokemonSinglesCollectionTab==="incoming")renderPokemonIncomingView();},true);}catch{}
}
...
function pokemonIncomingLabel(project,code,variant){
 if(!variant||variant==="fixed")return "Carta";
 const meta=project?.pokemonMeta?.[String(code)]||{};
 return pokemonBaseVariantOptions(project,String(code),meta).find(([key])=>key===variant)?.[1]||variant;
}
function pokemonSetIncoming(project,section,code,variant="fixed",enabled=true){
 if(!project)return;project.pokemonIncoming=project.pokemonIncoming||{};const key=pokemonIncomingKey(section,code,variant);
 if(enabled)project.pokemonIncoming[key]={section,code:String(code),variant:variant||"fixed",addedAt:new Date().toISOString()};else delete project.pokemonIncoming[key];
 persistProjects();scheduleCloudSave();
}
function pokemonToggleIncomingForCard(project,section,code,variant="fixed"){
 const enabled=!pokemonIncomingHas(project,section,code,variant);pokemonSetIncoming(project,section,code,variant,enabled);renderPokemonCollection();updateNavigationBadges();showToast(enabled?"📦 Marcada como en camino":"Quitada de En camino");
}
function pokemonIncomingEntries(){
 const rows=[];
 Object.values(projects||{}).forEach(project=>{
   const type=inferCollectionType(project);
   // Mis Singles es virtual: debe agregarse ANTES de filtrar los álbumes Pokémon normales.
   if(type==="pokemon-singles"){
     (Array.isArray(project.pokemonSingles)?project.pokemonSingles:[]).filter(card=>card?.status==="incoming"&&!card?.linkedAlbumProjectId).forEach(card=>rows.push({kind:"single",project,card,key:`single|||${project.id}|||${card.id}`}));
     return;
   }
   if(!isPokemonCollectionType(type))return;
   Object.entries(project.pokemonIncoming||{}).forEach(([incomingKey,item])=>rows.push({kind:"collection",project,key:`collection|||${project.id}|||${incomingKey}`,incomingKey,item}));
 });
 return rows.sort((a,b)=>String(b.item?.addedAt||b.card?.addedAt||"").localeCompare(String(a.item?.addedAt||a.card?.addedAt||"")));
}
function pokemonIncomingCardInfo(row){
 if(row.kind==="single")return {name:row.card.name||"Carta Pokémon",setName:row.card.setName||"Sin colección",number:row.card.number||"—",image:row.card.image||"",detail:[row.card.language?.toUpperCase(),row.card.rarity].filter(Boolean).join(" · ")};
 const {project,item}=row,type=inferCollectionType(project),meta=project?.pokemonMeta?.[String(item.code)]||{};
 return {name:meta.name||`Carta ${item.code}`,setName:project.name,number:item.code,image:meta.images?.small||pokemonDirectImageUrl(type,item.code,"small")||"",detail:pokemonIncomingLabel(project,item.code,item.variant)};
}
function pokemonReceiveIncoming(key){
 const row=pokemonIncomingEntries().find(entry=>entry.key===key);if(!row)return;
 if(row.kind==="single"){
   row.card.status="owned";row.card.receivedAt=new Date().toISOString();row.card.addedAt=row.card.addedAt||row.card.receivedAt;
 }else{
   const {project,item}=row;
   if(item.variant&&item.variant!=="fixed"){
     project.pokemonVariants=project.pokemonVariants||{};const state=project.pokemonVariants[item.code]||(project.pokemonVariants[item.code]={basic:0,holo:0,reverse:0,pattern:0});state[item.variant]=(Number(state[item.variant])||0)+1;
   }else{
     project.inventory=project.inventory||{};project.inventory[item.section]=project.inventory[item.section]||{};project.inventory[item.section][item.code]=(Number(project.inventory[item.section][item.code])||0)+1;
     if(project.id===activeProjectId){inventory[item.section]=inventory[item.section]||{};inventory[item.section][item.code]=(Number(inventory[item.section][item.code])||0)+1;}
   }
   delete project.pokemonIncoming[row.incomingKey];
   const mirror=pokemonSinglesFindMirror(project.id,item.section,item.code);if(mirror){mirror.status="owned";mirror.addedAt=new Date().toISOString();}
 }
 persistProjects();scheduleCloudSave();if(row.project.id===activeProjectId)renderAll();renderPokemonIncomingView();updateNavigationBadges();showToast("✓ Carta recibida y añadida");
}
function pokemonCancelIncoming(key){
 const row=pokemonIncomingEntries().find(entry=>entry.key===key);if(!row)return;
 if(row.kind==="single")row.project.pokemonSingles=(row.project.pokemonSingles||[]).filter(card=>card.id!==row.card.id);else{delete row.project.pokemonIncoming[row.incomingKey];const mirror=pokemonSinglesFindMirror(row.project.id,row.item.section,row.item.code);if(mirror?.status==="incoming"){if(pokemonSinglesCardOwnedInAlbum(row.project,row.item.section,row.item.code))mirror.status="owned";else pokemonSinglesProject().pokemonSingles=pokemonSinglesProject().pokemonSingles.filter(card=>card!==mirror);}}
...
function openPokemonSinglesCollection(){
 let project=pokemonSinglesProject();
 if(!project){project=defaultProject("Mis Singles",1,{},"pokemon-singles");project.collectionType="pokemon-singles";project.pokemonSingles=[];project.pokemonIncoming={};project.teamOrder=[];project.selectedTeam="";projects[project.id]=project;persistProjects();scheduleCloudSave();}
...
function pokemonSinglesCollectorKey(value){
 const raw=String(value||"").trim().toLowerCase().replace(/\s+/g,"");if(!raw)return "";const m=raw.match(/^([a-z]+)?0*(\d+)([a-z]*)$/i);if(!m)return raw.replace(/^0+(?=\d)/,"");return `${m[1]||""}${Number(m[2])}${m[3]||""}`;
}
function pokemonSinglesCollectorCandidates(card){
 const raw=String(card?.number||pokemonSinglesTcgdexParts(card).number||"").trim(),setId=pokemonSinglesCanonicalSetId(card?.setId||pokemonSinglesTcgdexParts(card).setId||""),out=[];const push=v=>{v=String(v||"").trim();if(v&&!out.includes(v))out.push(v)};push(raw);
 const pref=raw.match(/^([A-Za-z]+)0*(\d+)$/);if(pref){push(`${pref[1].toUpperCase()}${String(Number(pref[2])).padStart(2,"0")}`);push(String(Number(pref[2])));}
 const digits=raw.match(/(\d+)$/)?.[1]||"";if(digits){push(String(Number(digits)));push(String(Number(digits)).padStart(2,"0"));push(String(Number(digits)).padStart(3,"0"));if(setId==="mep")push(`MEP${String(Number(digits)).padStart(3,"0")}`);}
 return out;
}
function pokemonSinglesSetAffinity(card,row){
 const wanted=pokemonSinglesCanonicalSetId(card?.setId||pokemonSinglesTcgdexParts(card).setId||""),rowId=String(row?.set?.id||row?.id||"");const cut=rowId.lastIndexOf("-"),candidate=pokemonSinglesCanonicalSetId(row?.set?.id||(cut>0?rowId.slice(0,cut):""));if(wanted&&candidate===wanted)return 200;const ws=normalizeTradeName(card?.setName||""),rs=normalizeTradeName(row?.set?.name||"");if(ws&&rs&&(ws===rs||ws.includes(rs)||rs.includes(ws)))return 100;return 0;
}
async function pokemonSinglesResolveTcgdexDetail(card){
 const parts=pokemonSinglesTcgdexParts(card),langs=[card?.language||"es","en"].filter((v,i,a)=>v&&a.indexOf(v)===i),ids=[];const push=id=>{id=String(id||"").trim();if(id&&!ids.includes(id))ids.push(id)};push(parts.tcgdexId);
 const collector=pokemonSinglesCollectorCandidates(card);pokemonSinglesTcgdexSetCandidates(parts.setId||card?.setId||"").forEach(setId=>collector.forEach(local=>push(`${setId}-${local}`)));
 for(const lang of langs){
  for(const id of ids){try{const d=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/${encodeURIComponent(lang)}/cards/${encodeURIComponent(id)}`);if(d?.id&&pokemonSinglesCollectorKey(d.localId||d.id.split("-").pop())===pokemonSinglesCollectorKey(card?.number||parts.number))return {detail:d,lang};}catch{}}
  for(const localId of collector){try{const rows=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/${encodeURIComponent(lang)}/cards?localId=${encodeURIComponent(localId)}`);if(!Array.isArray(rows))continue;const wantedKey=pokemonSinglesCollectorKey(card?.number||parts.number),ranked=rows.filter(r=>pokemonSinglesCollectorKey(r?.localId||String(r?.id||"").split("-").pop())===wantedKey).map(r=>({r,score:pokemonSinglesSetAffinity(card,r)+(normalizeTradeName(r?.name||"")===normalizeTradeName(card?.name||"")?80:0)})).sort((a,b)=>b.score-a.score);for(const {r} of ranked.slice(0,3)){try{const d=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/${encodeURIComponent(lang)}/cards/${encodeURIComponent(r.id)}`);if(d?.id)return {detail:d,lang};}catch{}}}catch{}}
 }
 return null;
}
...
function pokemonSinglesScrydexImages(card){
 const parts=pokemonSinglesTcgdexParts(card),sets=pokemonSinglesTcgdexSetCandidates(parts.setId||card?.setId||""),numbers=pokemonSinglesCollectorCandidates(card),rows=[];for(const setId of sets){for(const number of numbers){rows.push({image:`https://images.scrydex.com/pokemon/${encodeURIComponent(setId)}-${encodeURIComponent(number)}/small`,imageLarge:`https://images.scrydex.com/pokemon/${encodeURIComponent(setId)}-${encodeURIComponent(number)}/large`});}}return rows;
}
...
function pokemonSinglesNormalizeCardmarket(cm){
 ...
 const trendHolo=pokemonSinglesPositiveNumber(prices["trend-holo"]??prices.foilTrend),avg7Holo=pokemonSinglesPositiveNumber(prices["avg7-holo"]??prices.foilAvg7),avg30Holo=pokemonSinglesPositiveNumber(prices["avg30-holo"]??prices.foilAvg30),avgHolo=pokemonSinglesPositiveNumber(prices["avg-holo"]??prices.foilSell),lowHolo=pokemonSinglesPositiveNumber(prices["low-holo"]??prices.foilLow);
 if(![trend,avg7,avg30,avg,low,trendHolo,avg7Holo,avg30Holo,avgHolo,lowHolo].some(v=>v!==null))return null;
 return {trend,avg7,avg30,avg,low,"trend-holo":trendHolo,"avg7-holo":avg7Holo,"avg30-holo":avg30Holo,"avg-holo":avgHolo,"low-holo":lowHolo,updated:cm.updatedAt||cm.updated||cm.createdAt||"",url:cm.url||"",productId:cm.productId||cm.idProduct||null,source:cm.source||"",fetchedAt:cm.fetchedAt||""};
}
function pokemonSinglesCardmarketRows(payload){if(Array.isArray(payload))return payload;for(const key of ["products","priceGuide","prices","data","items"]){if(Array.isArray(payload?.[key]))return payload[key];}return []}
function pokemonSinglesCardmarketProductId(row){return String(row?.idProduct??row?.id_product??row?.productId??row?.id??"")}
function pokemonSinglesCardmarketOpenDb(){return new Promise((resolve,reject)=>{if(typeof indexedDB==="undefined"){resolve(null);return;}const req=indexedDB.open(POKEMON_CARDMARKET_IDB_NAME,1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(POKEMON_CARDMARKET_IDB_STORE))db.createObjectStore(POKEMON_CARDMARKET_IDB_STORE);};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error||new Error("IndexedDB unavailable"));});}
async function pokemonSinglesCardmarketReadCache(){try{const db=await pokemonSinglesCardmarketOpenDb();if(!db)return null;return await new Promise((resolve,reject)=>{const tx=db.transaction(POKEMON_CARDMARKET_IDB_STORE,"readonly"),req=tx.objectStore(POKEMON_CARDMARKET_IDB_STORE).get(POKEMON_CARDMARKET_IDB_KEY);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error);tx.oncomplete=()=>db.close();});}catch(error){console.warn("Cardmarket cache read",error);return null;}}
async function pokemonSinglesCardmarketWriteCache(snapshot){try{const db=await pokemonSinglesCardmarketOpenDb();if(!db)return false;await new Promise((resolve,reject)=>{const tx=db.transaction(POKEMON_CARDMARKET_IDB_STORE,"readwrite");tx.objectStore(POKEMON_CARDMARKET_IDB_STORE).put(snapshot,POKEMON_CARDMARKET_IDB_KEY);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});db.close();return true;}catch(error){console.warn("Cardmarket cache write",error);return false;}}
function pokemonSinglesCardmarketReduceProduct(row){const id=pokemonSinglesCardmarketProductId(row);if(!id)return null;return {idProduct:id,name:row?.name??row?.Name??"",expansionName:row?.expansionName??row?.Expansion??row?.expansion??row?.categoryName??"",number:row?.number??row?.Number??row?.collectorNumber??"",idExpansion:row?.idExpansion??row?.expansionId??null};}
function pokemonSinglesCardmarketReducePrice(row){const id=pokemonSinglesCardmarketProductId(row);if(!id)return null;return {idProduct:id,trendPrice:row?.trendPrice??row?.trend??null,avg1:row?.avg1??null,avg7:row?.avg7??null,avg30:row?.avg30??null,averageSellPrice:row?.averageSellPrice??row?.avg??null,lowPrice:row?.lowPrice??row?.low??null,"trend-holo":row?.["trend-holo"]??row?.foilTrend??null,"avg7-holo":row?.["avg7-holo"]??row?.foilAvg7??null,"avg30-holo":row?.["avg30-holo"]??row?.foilAvg30??null,"avg-holo":row?.["avg-holo"]??row?.foilSell??null,"low-holo":row?.["low-holo"]??row?.foilLow??null};}
function pokemonSinglesCardmarketHydrateSnapshot(snapshot){if(!snapshot?.products?.length||!snapshot?.prices?.length)return null;const priceById=new Map();snapshot.prices.forEach(row=>{const id=pokemonSinglesCardmarketProductId(row);if(id)priceById.set(id,row)});return {products:snapshot.products,priceById,createdAt:snapshot.createdAt||"",fetchedAt:snapshot.fetchedAt||"",cachedAt:Number(snapshot.cachedAt)||0};}
async function pokemonSinglesCardmarketFetchJson(url){let last=null;for(let attempt=0;attempt<2;attempt++){const controller=typeof AbortController!=="undefined"?new AbortController():null,timer=controller?setTimeout(()=>controller.abort(),POKEMON_CARDMARKET_FETCH_TIMEOUT_MS):null;try{const response=await fetch(url,{cache:"no-store",signal:controller?.signal});if(!response.ok)throw new Error(`HTTP ${response.status}`);return await response.json();}catch(error){last=error;if(attempt===0)await new Promise(resolve=>setTimeout(resolve,650));}finally{if(timer)clearTimeout(timer);}}throw last||new Error("Cardmarket unavailable");}
async function pokemonSinglesRefreshCardmarketData(){if(pokemonSinglesCardmarketRefreshPromise)return pokemonSinglesCardmarketRefreshPromise;pokemonSinglesCardmarketRefreshPromise=(async()=>{try{const [catalogPayload,pricePayload]=await Promise.all([pokemonSinglesCardmarketFetchJson(CARDMARKET_POKEMON_PRODUCTS_URL),pokemonSinglesCardmarketFetchJson(CARDMARKET_POKEMON_PRICES_URL)]),products=pokemonSinglesCardmarketRows(catalogPayload).map(pokemonSinglesCardmarketReduceProduct).filter(Boolean),prices=pokemonSinglesCardmarketRows(pricePayload).map(pokemonSinglesCardmarketReducePrice).filter(Boolean);if(!products.length||!prices.length)throw new Error("Cardmarket payload empty");const snapshot={schema:1,products,prices,createdAt:pricePayload?.createdAt||"",fetchedAt:new Date().toISOString(),cachedAt:Date.now()};await pokemonSinglesCardmarketWriteCache(snapshot);const hydrated=pokemonSinglesCardmarketHydrateSnapshot(snapshot);pokemonSinglesCardmarketDataLoadedAt=Date.now();pokemonSinglesCardmarketDataPromise=Promise.resolve(hydrated);pokemonSinglesCardmarketCacheReadPromise=Promise.resolve(hydrated);pokemonSinglesCardmarketMatchCache.clear();setTimeout(()=>pokemonSinglesHydrateVisibleSearchPrices(),0);return hydrated;}catch(error){console.warn("Cardmarket public data refresh",error);return null;}finally{pokemonSinglesCardmarketRefreshPromise=null;}})();return pokemonSinglesCardmarketRefreshPromise;}
async function pokemonSinglesLoadCardmarketCachedOnly(){
 const now=Date.now();
 if(pokemonSinglesCardmarketDataPromise&&pokemonSinglesCardmarketDataLoadedAt)return pokemonSinglesCardmarketDataPromise;
 if(pokemonSinglesCardmarketCacheReadPromise)return pokemonSinglesCardmarketCacheReadPromise;
 pokemonSinglesCardmarketCacheReadPromise=(async()=>{const cached=await pokemonSinglesCardmarketReadCache(),hydrated=pokemonSinglesCardmarketHydrateSnapshot(cached);if(hydrated){pokemonSinglesCardmarketDataLoadedAt=Number(cached.cachedAt)||now;pokemonSinglesCardmarketDataPromise=Promise.resolve(hydrated);if((now-pokemonSinglesCardmarketDataLoadedAt)>=POKEMON_CARDMARKET_REFRESH_MS)void pokemonSinglesRefreshCardmarketData();return hydrated;}void pokemonSinglesRefreshCardmarketData();return null;})();
 return pokemonSinglesCardmarketCacheReadPromise;
}
async function pokemonSinglesLoadCardmarketData(force=false){
 const now=Date.now();
 if(!force&&pokemonSinglesCardmarketDataPromise&&pokemonSinglesCardmarketDataLoadedAt)return pokemonSinglesCardmarketDataPromise;
 const cached=await pokemonSinglesLoadCardmarketCachedOnly();if(cached&&!force)return cached;
 const fresh=await pokemonSinglesRefreshCardmarketData();if(fresh)return fresh;
 return cached||null;
}
function pokemonSinglesPrimeCardmarketData(){void pokemonSinglesLoadCardmarketCachedOnly();}
function pokemonSinglesCardmarketNumberTokens(value){return String(value||"").toLowerCase().match(/[a-z]*\d+[a-z]*/g)||[]}
function pokemonSinglesCardmarketProductScore(product,card,englishName=""){
 const pName=normalizeTradeName(product?.name??product?.Name??""),pExpansion=normalizeTradeName(product?.expansionName??product?.Expansion??product?.expansion??product?.categoryName??""),wantedName=normalizeTradeName(englishName||card?.name||""),localName=normalizeTradeName(card?.name||""),wantedSet=normalizeTradeName(card?.setName||""),number=String(card?.number||"").replace(/^0+(?=\d)/,"").toLowerCase();let score=0;
 if(wantedName&&pName===wantedName)score+=180;else if(wantedName&&pName.includes(wantedName))score+=100;if(localName&&localName!==wantedName&&pName.includes(localName))score+=55;if(wantedSet&&pExpansion===wantedSet)score+=120;else if(wantedSet&&pExpansion&&(pExpansion.includes(wantedSet)||wantedSet.includes(pExpansion)))score+=65;
 const tokens=pokemonSinglesCardmarketNumberTokens(product?.number??product?.Number??product?.collectorNumber??product?.name??product?.Name);if(number&&tokens.some(token=>token.replace(/^0+(?=\d)/,"")===number))score+=220;
 return score;
}
async function pokemonSinglesCardmarketDirect(card,cacheOnly=false){
 const cacheKey=`${card?.tcgdexId||card?.id}|${card?.language||""}|${cacheOnly?"cache":"full"}`;if(pokemonSinglesCardmarketMatchCache.has(cacheKey))return pokemonSinglesCardmarketMatchCache.get(cacheKey);
 const promise=(async()=>{try{
  const data=cacheOnly?await pokemonSinglesLoadCardmarketCachedOnly():await pokemonSinglesLoadCardmarketData();if(!data?.products?.length)return null;let englishName=String(card?.englishName||"");if(!englishName){if(cacheOnly&&card?.language==="en")englishName=String(card?.name||"");else{const identity=await pokemonSinglesEnglishIdentity(card);englishName=identity.name||String(card?.name||"");}}
...
async function pokemonSinglesPokemonTcgFallback(card){
 const key=`${String(card?.name||"").toLowerCase()}|${String(card?.number||"")}|${String(card?.setName||"").toLowerCase()}`;if(pokemonSinglesPtcgCache.has(key))return pokemonSinglesPtcgCache.get(key);
 const promise=(async()=>{try{
   let searchName=String(card?.name||"");const {tcgdexId}=pokemonSinglesTcgdexParts(card);if(card?.language!=="en"&&tcgdexId){try{const en=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/en/cards/${encodeURIComponent(tcgdexId)}`);if(en?.name)searchName=en.name;}catch{}}const name=searchName.replace(/["\\]/g," ").trim(),wantedNumber=pokemonSinglesCollectorKey(card?.number||"");if(!name||!wantedNumber)return null;
   ...
   const numberMatches=value=>pokemonSinglesCollectorKey(value)===wantedNumber;const sameNumber=rows.filter(row=>numberMatches(row?.number));const wantedSet=normalizeTradeName(card?.setName||"");sameNumber.sort((a,b)=>{const score=row=>{const set=normalizeTradeName(row?.set?.name||"");let n=0;if(set===wantedSet)n+=100;if(set.includes(wantedSet)||wantedSet.includes(set))n+=50;return n};return score(b)-score(a)});const hit=sameNumber[0]||null;if(!hit)return null;return {image:hit?.images?.small||hit?.images?.large||"",imageLarge:hit?.images?.large||hit?.images?.small||"",cardmarket:pokemonSinglesNormalizeCardmarket(hit?.cardmarket||null),ptcgId:hit?.id||""};
  }catch{return null}})();pokemonSinglesPtcgCache.set(key,promise);return promise;
}
async function pokemonSinglesEnsureCardImage(project,card,onUpdated,force=false){
 if(!project||!card)return "";
 const failed=pokemonSinglesBrokenImageUrls(card),current=String(card.image||"").trim(),currentLarge=String(card.imageLarge||"").trim();
 if(!force&&current&&currentLarge&&!failed.has(current)&&!failed.has(currentLarge))return current;
 const {tcgdexId}=pokemonSinglesTcgdexParts(card),hydrationKey=`${project.id}|||${card.id||tcgdexId||`${card.setId||card.setName}:${card.number||""}`}`;if(pokemonSinglesImageHydration.has(hydrationKey))return card.image||"";pokemonSinglesImageHydration.add(hydrationKey);
 const valid=async url=>{url=String(url||"").trim();if(!url||failed.has(url))return false;return pokemonSinglesProbeImage(url)};
 ...
 try{
  ...
  if(current&&!legacyMepBack&&!failed.has(current)&&await pokemonSinglesProbeImage(current)){
   ...
   card.imageLarge=current;card.imageSchema=POKEMON_SINGLES_IMAGE_SCHEMA;persistProjects();scheduleCloudSave();return current;
  }
  if(current)failed.add(current);if(currentLarge)failed.add(currentLarge);
  // 1) Imagen conocida del propio álbum/checklist. Especialmente importante para First Partner 046-063.
  const albumSmall=pokemonSinglesKnownAlbumImage(card,"small"),albumLarge=pokemonSinglesKnownAlbumImage(card,"large");if(albumSmall){const hit=await commit(albumSmall,albumLarge||albumSmall,"album-known");if(hit)return hit;}
  // 2) Resolver TCGdex por identidad real. Para TG/GG/MEP conservamos el collector number completo (TG03/GG05/032) y no solo sus dígitos.
...
function pokemonSinglesHydrateMissingImages(project){(project?.pokemonSingles||[]).filter(card=>!card.image||!card.imageLarge||Number(card.imageSchema||0)<POKEMON_SINGLES_IMAGE_SCHEMA).forEach(card=>pokemonSinglesEnsureCardImage(project,card,()=>{if(project.id===activeProjectId&&mainTab==="collection")renderPokemonSinglesCollection();},Number(card.imageSchema||0)<POKEMON_SINGLES_IMAGE_SCHEMA));}
function pokemonSinglesLanguageLabel(code){return ({es:"ES",en:"EN",ja:"JP","zh-cn":"CN","zh-tw":"TW"})[code]||String(code||"").toUpperCase()}
function pokemonSinglesCanonicalSetId(value){
 let id=String(value||"").trim().toLowerCase().replace(/[_\s-]+/g,"");
 id=id.replace(/\.5/g,"pt5").replace(/\./g,"");
 id=id.replace(/^([a-z]+)0+(\d)/,"$1$2");
 return id;
}
function pokemonSinglesCollectionTarget(card){
 const setId=pokemonSinglesCanonicalSetId(card?.setId),number=Number(String(card?.number||"").replace(/\D/g,""));if(!setId||!number)return null;
 for(const project of Object.values(projects||{})){
  const type=inferCollectionType(project),def=POKEMON_SET_DEFS[type];if(!def||def.virtual||pokemonSinglesCanonicalSetId(def.setId)!==setId)continue;
  for(const [section] of def.ranges){const code=pokemonCode(number);if(Object.prototype.hasOwnProperty.call(project?.inventory?.[section]||{},code)){
    const meta=project?.pokemonMeta?.[code]||{},ex=section==="BASE"&&pokemonIsEx(meta),options=(section==="BASE"&&!ex)?pokemonBaseVariantOptions(project,code,meta):[];
    return {project,type,def,section,code,meta,options};
  }}
 }
 return null;
}
function pokemonSinglesFindMirror(projectId,section,code){
 const singles=pokemonSinglesProject();if(!singles)return null;return (singles.pokemonSingles||[]).find(card=>card.linkedAlbumProjectId===projectId&&card.linkedSection===section&&String(card.linkedCode)===String(code))||null;
}
function pokemonSinglesTargetFromMirror(card){const project=projects?.[card?.linkedAlbumProjectId];if(!project)return null;const type=inferCollectionType(project),def=POKEMON_SET_DEFS[type],section=card?.linkedSection,code=String(card?.linkedCode||"");if(!def||!section||!code)return null;const meta=project?.pokemonMeta?.[code]||{},ex=section==="BASE"&&pokemonIsEx(meta),options=(section==="BASE"&&!ex)?pokemonBaseVariantOptions(project,code,meta):[];return {project,type,def,section,code,meta,options};}
function pokemonSinglesUpsertMirror(card,target,status="owned",extra={}){
 const singles=pokemonSinglesProject();if(!singles||!target)return null;singles.pokemonSingles=Array.isArray(singles.pokemonSingles)?singles.pokemonSingles:[];
 let row=pokemonSinglesFindMirror(target.project.id,target.section,target.code)||singles.pokemonSingles.find(item=>item.id===card.id);
 // Un mirror existente ya puede contener identidad canónica, imagen HD y Cardmarket.
 // Los stubs del álbum solo sirven para crear la ficha; nunca deben volver a pisar esos datos.
 const source=row?(extra.manualKeep?{...row,...card}:{...row}):{...card};
 const base={...source,status:status==="incoming"?"incoming":"owned",linkedAlbumProjectId:target.project.id,linkedAlbumName:target.project.name,linkedSection:target.section,linkedCode:String(target.code),routeVariant:extra.variant||card.routeVariant||row?.routeVariant||"fixed",mirror:true,addedAt:row?.addedAt||new Date().toISOString(),...extra};
 if(row)Object.assign(row,base);else{row=base;singles.pokemonSingles.push(row);}return row;
}
...
function pokemonSinglesMirrorNeedsCanonicalRepair(card){
 if(!card?.mirror||!card?.linkedAlbumProjectId)return false;
 if(Number(card.canonicalSchema||0)<POKEMON_SINGLES_CANONICAL_SCHEMA)return true;
 if(!card.tcgdexId||!card.image||!card.imageLarge||Number(card.imageSchema||0)<POKEMON_SINGLES_IMAGE_SCHEMA)return true;
 return false;
}
...
async function pokemonSinglesCanonicalizeMirror(card){
 const target=pokemonSinglesTargetFromMirror(card);if(!target)return false;
 const found=await pokemonSinglesCanonicalDetail(card,target);card.canonicalRepairAttemptAt=new Date().toISOString();if(!found?.detail)return false;
 const d=found.detail,base=String(d.image||"").replace(/\/$/,""),low=pokemonSinglesImageUrl(base,"low"),high=pokemonSinglesImageUrl(base,"high"),normalizedPricing=pokemonSinglesNormalizeCardmarket(d?.pricing?.cardmarket||null);
 card.tcgdexId=d.id||card.tcgdexId;card.setId=d?.set?.id||card.setId;card.setName=target.project.name||d?.set?.name||card.setName;if(found.lang===(card.language||"es")||!card.name||/^Carta\s+\d+$/i.test(card.name))card.name=d.name||card.name;card.number=String(d.localId||card.number||Number(target.code));card.rarity=d.rarity||card.rarity;card.language=card.language||"es";card.variants=d.variants||card.variants||{};
 if(base)card.imageBase=base; // La imagen canónica se valida/persiste exclusivamente en pokemonSinglesEnsureCardImage().
 // TCGdex ya aporta Cardmarket en muchos sets; es el mismo fallback usado por el buscador de Singles.
 if(normalizedPricing)card.cardmarket=normalizedPricing;
 const direct=await pokemonSinglesCardmarketDirect(card);if(direct)card.cardmarket={...direct,fetchedAt:new Date().toISOString()};
 if(card.cardmarket)card.cardmarketFetchedAt=new Date().toISOString();
 card.canonicalSchema=POKEMON_SINGLES_CANONICAL_SCHEMA;card.canonicalHydratedAt=new Date().toISOString();const singles=pokemonSinglesProject();if(singles)await pokemonSinglesEnsureCardImage(singles,card,null,Number(card.imageSchema||0)<POKEMON_SINGLES_IMAGE_SCHEMA);return true;
}
async function pokemonSinglesRepairExistingMirrors(project){
 if(pokemonSinglesCanonicalRepairInFlight||!project||inferCollectionType(project)!=="pokemon-singles")return;
 const now=Date.now(),cards=(project.pokemonSingles||[]).filter(card=>{if(!pokemonSinglesMirrorNeedsCanonicalRepair(card))return false;const last=Date.parse(card?.canonicalRepairAttemptAt||"");return !Number.isFinite(last)||(now-last)>6*60*60*1000;});if(!cards.length)return;
 pokemonSinglesCanonicalRepairInFlight=true;let changed=false;
 try{for(let i=0;i<cards.length;i+=4){const batch=cards.slice(i,i+4),results=await Promise.all(batch.map(card=>pokemonSinglesCanonicalizeMirror(card).catch(()=>false)));if(results.some(Boolean))changed=true;}if(changed){persistProjects();scheduleCloudSave();if(project.id===activeProjectId&&mainTab==="collection")requestAnimationFrame(()=>renderPokemonSinglesCollection());}}
 finally{pokemonSinglesCanonicalRepairInFlight=false;}
}
function pokemonSinglesCardOwnedInAlbum(project,section,code){
 ...
 const meta=project?.pokemonMeta?.[code]||{},ex=pokemonIsEx(meta),options=!ex?pokemonBaseVariantOptions(project,code,meta):[];
 if(options.length)return options.some(([key])=>(Number(project?.pokemonVariants?.[code]?.[key])||0)>0);
 return (Number(project?.inventory?.[section]?.[code])||0)>0;
}
function pokemonSinglesSameAlbumReference(card,project,section,code){
 if(!card||!project)return false;const linked=card.linkedAlbumProjectId===project.id&&card.linkedSection===section&&String(card.linkedCode)===String(code);if(linked)return true;const target=pokemonSinglesCollectionTarget(card);return Boolean(target&&target.project?.id===project.id&&target.section===section&&String(target.code)===String(code));
}
function pokemonSinglesClearIncomingReference(project,section,code){
 if(!project)return false;let changed=false;project.pokemonIncoming=project.pokemonIncoming||{};for(const [key,row] of Object.entries(project.pokemonIncoming)){if(row?.section===section&&String(row?.code)===String(code)){delete project.pokemonIncoming[key];changed=true;}}return changed;
}
function pokemonSinglesRemoveAlbumReferenceEverywhere(project,section,code){
 if(!project)return false;const singles=pokemonSinglesProject();let changed=pokemonSinglesClearIncomingReference(project,section,code);if(singles&&Array.isArray(singles.pokemonSingles)){const before=singles.pokemonSingles.length;singles.pokemonSingles=singles.pokemonSingles.filter(card=>!pokemonSinglesSameAlbumReference(card,project,section,code));if(singles.pokemonSingles.length!==before)changed=true;}return changed;
}
function pokemonSinglesDeleteEverywhere(card){
 const singles=pokemonSinglesProject();if(!singles||!card)return false;const target=pokemonSinglesTargetFromMirror(card)||pokemonSinglesCollectionTarget(card);let changed=false;if(target){const {project,section,code,options}=target;if(options?.length){const state=pokemonVariantState(project,code);for(const [key] of options){if(Number(state[key])>0){state[key]=0;changed=true;}}}else{project.inventory=project.inventory||{};project.inventory[section]=project.inventory[section]||{};if(Number(project.inventory[section][code])>0){project.inventory[section][code]=0;changed=true;}}if(pokemonSinglesClearIncomingReference(project,section,code))changed=true;const before=singles.pokemonSingles.length;singles.pokemonSingles=singles.pokemonSingles.filter(item=>!pokemonSinglesSameAlbumReference(item,project,section,code));if(singles.pokemonSingles.length!==before)changed=true;}else{const before=singles.pokemonSingles.length;singles.pokemonSingles=singles.pokemonSingles.filter(item=>item.id!==card.id);if(singles.pokemonSingles.length!==before)changed=true;}return changed;
}
async function pokemonSinglesChangeOwnedLanguage(card,language){
 const project=pokemonSinglesProject();if(!project||!card)return;language=String(language||"").trim();if(!language||card.language===language)return;card.language=language;card.canonicalSchema=0;card.imageSchema=0;card.image="";card.imageLarge="";card.imageBase="";card.imageFailedUrls=[];card.imageSource="";persistProjects();scheduleCloudSave();renderPokemonSinglesCollection();try{const found=await pokemonSinglesResolveTcgdexDetail(card);if(found?.detail){const d=found.detail,base=String(d.image||"").replace(/\/$/,"");card.tcgdexId=d.id||card.tcgdexId;card.name=d.name||card.name;card.number=String(d.localId||card.number||"");card.rarity=d.rarity||card.rarity;card.variants=d.variants||card.variants||{};if(base)card.imageBase=base;const price=pokemonSinglesNormalizeCardmarket(d?.pricing?.cardmarket||null);if(price){card.cardmarket=price;card.cardmarketFetchedAt=new Date().toISOString();}}await pokemonSinglesEnsureCardImage(project,card,null,true);}catch(error){console.warn("Pokemon single language refresh",error)}finally{persistProjects();scheduleCloudSave();if(project.id===activeProjectId&&mainTab==="collection")renderPokemonSinglesCollection();}
}
function pokemonSinglesMirrorShouldStay(card){return !card?.autoManaged||card?.manualKeep===true||card?.status==="incoming";}
async function pokemonSinglesSyncOwnedCollections(force=false){
 if(pokemonSinglesAutoSyncInFlight)return;const singles=pokemonSinglesProject();if(!singles)return;pokemonSinglesAutoSyncInFlight=true;let changed=false;
 try{
  const scanStamp=Date.parse(singles.autoValueScanAt||""),priceScanDue=force||!Number.isFinite(scanStamp)||(Date.now()-scanStamp)>=POKEMON_CARDMARKET_REFRESH_MS;const seen=new Set(),baseCandidates=[];
  for(const project of Object.values(projects||{})){
   const type=inferCollectionType(project),def=POKEMON_SET_DEFS[type];if(!def||def.virtual)continue;
   for(const [section,start,end] of def.ranges){for(let n=start;n<=end;n++){const code=pokemonCode(n);if(!pokemonSinglesCardOwnedInAlbum(project,section,code))continue;const key=`${project.id}|${section}|${code}`;seen.add(key);const stub=pokemonSinglesAlbumCardStub(project,type,section,code),target={project,type,def,section,code,meta:project?.pokemonMeta?.[code]||{},options:[]},existing=pokemonSinglesFindMirror(project.id,section,code);
    if(section!=="BASE"){
...
function pokemonSinglesMoveExistingToAlbums(singlesProject){
 // Mis Singles es un índice paralelo: si hay álbum compatible, la carta debe existir allí y conservar su ficha aquí.
 if(!singlesProject||inferCollectionType(singlesProject)!=="pokemon-singles"||!Array.isArray(singlesProject.pokemonSingles))return 0;let linked=0,changed=false;
 for(const card of singlesProject.pokemonSingles){const target=pokemonSinglesCollectionTarget(card);if(!target)continue;const {project,section,code,options}=target,variant=card.routeVariant&&options.some(([key])=>key===card.routeVariant)?card.routeVariant:(options[0]?.[0]||"fixed");
  if(card.status==="incoming"){if(!pokemonIncomingHas(project,section,code,variant)){pokemonSetIncoming(project,section,code,variant,true);changed=true;}}
  else if(!pokemonSinglesCardOwnedInAlbum(project,section,code)){if(options.length){const state=pokemonVariantState(project,code);state[variant]=Math.max(1,Number(state[variant])||0);}else{project.inventory=project.inventory||{};project.inventory[section]=project.inventory[section]||{};project.inventory[section][code]=Math.max(1,Number(project.inventory[section][code])||0);}changed=true;}
  card.linkedAlbumProjectId=project.id;card.linkedAlbumName=project.name;card.linkedSection=section;card.linkedCode=String(code);card.routeVariant=variant;card.mirror=true;linked++;
 }
 if(changed){persistProjects();scheduleCloudSave();}return linked;
}
function pokemonSinglesPriceNeedsRefresh(card){
 const stamp=Date.parse(card?.cardmarketFetchedAt||card?.cardmarket?.fetchedAt||card?.cardmarket?.updated||"");
 return !Number.isFinite(stamp)||(Date.now()-stamp)>=POKEMON_CARDMARKET_REFRESH_MS;
}
async function pokemonSinglesRefreshStoredPrices(project,force=false){
 if(pokemonSinglesStoredPriceRefreshInFlight||!project||inferCollectionType(project)!=="pokemon-singles")return;
 const cards=(project.pokemonSingles||[]).filter(card=>force||pokemonSinglesPriceNeedsRefresh(card));if(!cards.length)return;
 pokemonSinglesStoredPriceRefreshInFlight=true;
 try{
  await pokemonSinglesLoadCardmarketData(force);let changed=false;
...
function pokemonSinglesRouteVariant(card,target){
 if(!target?.options?.length)return "fixed";const select=document.querySelector(`[data-single-variant="${CSS.escape(card.id)}"]`);const wanted=select?.value;return target.options.some(([key])=>key===wanted)?wanted:target.options[0][0];
}
function pokemonSinglesAddToCollection(card,status,target){
 const {project,section,code,options}=target,variant=pokemonSinglesRouteVariant(card,target),incoming=status==="incoming";
 if(incoming){if(pokemonIncomingHas(project,section,code,variant)){showToast(`Esta carta ya está En camino en ${project.name}`);return;}pokemonSetIncoming(project,section,code,variant,true);}
 else if(options.length){const state=pokemonVariantState(project,code);state[variant]=(Number(state[variant])||0)+1;}
 else{project.inventory=project.inventory||{};project.inventory[section]=project.inventory[section]||{};project.inventory[section][code]=(Number(project.inventory[section][code])||0)+1;}
 const mirror=pokemonSinglesUpsertMirror({...card,routeVariant:variant},target,incoming?"incoming":"owned",{manualKeep:true,variant,cardmarketFetchedAt:card.cardmarket?new Date().toISOString():""});if(mirror&&card.cardmarket)mirror.cardmarket=card.cardmarket;
 persistProjects();scheduleCloudSave();renderPokemonSinglesSearchResults();updateNavigationBadges();showToast(incoming?`📦 Añadida a En camino · ${project.name} y guardada en Mis Singles`:`✓ Añadida a ${project.name} y a Mis Singles`);
}
function pokemonSinglesIncomingListHtml(){
 const rows=pokemonIncomingEntries();if(!rows.length)return `<div class="pokemon-singles-empty">No tienes cartas en camino.</div>`;
 return `<div class="pokemon-singles-incoming-list">${rows.map((row,index)=>{const info=pokemonIncomingCardInfo(row),clickable=Boolean(info.image);return `<article class="pokemon-incoming-card"><div class="pokemon-incoming-thumb ${clickable?"is-clickable":""}" ${clickable?`data-inline-incoming-preview="${index}" role="button" tabindex="0"`:""}>${info.image?`<img src="${collectionSafeText(info.image)}" alt="${collectionSafeText(info.name)}" loading="lazy">`:'<span>PK</span>'}</div><div class="pokemon-incoming-copy"><small>${collectionSafeText(info.setName)}</small><strong>${collectionSafeText(info.name)}</strong><span>#${collectionSafeText(info.number)}${info.detail?` · ${collectionSafeText(info.detail)}`:""}</span></div><div class="pokemon-incoming-actions"><button type="button" data-pokemon-receive="${collectionSafeText(row.key)}">✓ Recibida</button><button type="button" data-pokemon-cancel-incoming="${collectionSafeText(row.key)}">Quitar</button></div></article>`}).join("")}</div>`;
}
function wirePokemonSinglesIncomingInline(){
 const rows=pokemonIncomingEntries();document.querySelectorAll('[data-pokemon-receive]').forEach(b=>b.onclick=()=>{pokemonReceiveIncoming(b.dataset.pokemonReceive);pokemonSinglesCollectionTab="incoming";setTimeout(renderPokemonSinglesCollection,0);});document.querySelectorAll('[data-pokemon-cancel-incoming]').forEach(b=>b.onclick=()=>{pokemonCancelIncoming(b.dataset.pokemonCancelIncoming);pokemonSinglesCollectionTab="incoming";setTimeout(renderPokemonSinglesCollection,0);});document.querySelectorAll('[data-inline-incoming-preview]').forEach(el=>el.onclick=()=>{const row=rows[Number(el.dataset.inlineIncomingPreview)],info=row&&pokemonIncomingCardInfo(row);if(info?.image)openPokemonCardImageViewer(info.image,info.number,{name:info.name});});
}
function renderPokemonSinglesCollection(){
 ...
 const dashboard=$("#pokemonDashboard");if(dashboard)dashboard.hidden=true;document.body.classList.add("pokemon-ui-active");project.pokemonSingles=Array.isArray(project.pokemonSingles)?project.pokemonSingles:[];pokemonSinglesMoveExistingToAlbums(project);pokemonSinglesRepairExistingMirrors(project);pokemonSinglesHydrateMissingImages(project);pokemonSinglesRefreshStoredPrices(project);pokemonSinglesSyncOwnedCollections();
 const ownedCards=project.pokemonSingles.filter(c=>c.status!=="incoming"),owned=ownedCards.length,incoming=pokemonIncomingEntries().length;
 const body=pokemonSinglesCollectionTab==="incoming"?pokemonSinglesIncomingListHtml():`<section class="pokemon-singles-list"><div class="pokemon-singles-list-head"><strong>Cartas</strong><span>${owned}</span></div>${ownedCards.length?ownedCards.slice().sort((a,b)=>String(b.addedAt||"").localeCompare(String(a.addedAt||""))).map(card=>pokemonSingleOwnedCardHtml(card)).join(""):'<div class="pokemon-singles-empty">Todavía no has registrado singles.</div>'}</section>`;
 list.innerHTML=`<section class="pokemon-singles-shell pokemon-singles-shell-compact"><div class="pokemon-singles-inner-tabs"><button type="button" data-singles-tab="owned" class="${pokemonSinglesCollectionTab==="owned"?"active":""}">★ Colección <em>${owned}</em></button><button type="button" data-singles-tab="incoming" class="${pokemonSinglesCollectionTab==="incoming"?"active":""}">📦 En camino <em>${incoming}</em></button></div>${body}</section>`;
 document.querySelectorAll('[data-singles-tab]').forEach(button=>button.onclick=()=>{pokemonSinglesCollectionTab=button.dataset.singlesTab||"owned";renderPokemonSinglesCollection();});wirePokemonSingleCards();wirePokemonSinglesIncomingInline();
}

function pokemonSinglesScanSetStatus(title,detail="",progress=null){
 const panel=$("#pokemonSingleScanPanel"),titleEl=$("#pokemonSingleScanTitle"),detailEl=$("#pokemonSingleScanDetail"),bar=$("#pokemonSingleScanProgress");if(!panel)return;
 panel.hidden=false;if(titleEl)titleEl.textContent=title;if(detailEl)detailEl.textContent=detail||"";if(bar){bar.hidden=progress===null;bar.style.setProperty("--scan-progress",`${Math.max(0,Math.min(100,Number(progress)||0))}%`);}
}
function pokemonSinglesLoadTesseract(){
 if(window.Tesseract?.createWorker)return Promise.resolve(window.Tesseract);if(pokemonSinglesTesseractPromise)return pokemonSinglesTesseractPromise;
 pokemonSinglesTesseractPromise=new Promise((resolve,reject)=>{const existing=document.querySelector('script[data-stickerbase-tesseract]');if(existing){existing.addEventListener("load",()=>window.Tesseract?.createWorker?resolve(window.Tesseract):reject(new Error("OCR no disponible")),{once:true});existing.addEventListener("error",()=>reject(new Error("No se pudo cargar OCR")),{once:true});return;}const script=document.createElement("script");script.src=POKEMON_SINGLES_TESSERACT_URL;script.async=true;script.dataset.stickerbaseTesseract="1";script.onload=()=>window.Tesseract?.createWorker?resolve(window.Tesseract):reject(new Error("OCR no disponible"));script.onerror=()=>reject(new Error("No se pudo cargar OCR"));document.head.appendChild(script);}).catch(error=>{pokemonSinglesTesseractPromise=null;throw error;});
 return pokemonSinglesTesseractPromise;
}
async function pokemonSinglesScannerWorker(){
 if(pokemonSinglesTesseractWorker)return pokemonSinglesTesseractWorker;const Tesseract=await pokemonSinglesLoadTesseract();
 pokemonSinglesTesseractWorker=await Tesseract.createWorker("eng",1,{logger:message=>{if(!pokemonSinglesScanBusy||message?.status!=="recognizing text")return;pokemonSinglesScanSetStatus("Identificando carta…","Leyendo número de colección y código del set",Math.round((Number(message.progress)||0)*100));}});
 try{await pokemonSinglesTesseractWorker.setParameters({tessedit_char_whitelist:"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz/- ",preserve_interword_spaces:"1"});}catch{}
 return pokemonSinglesTesseractWorker;
}
function pokemonSinglesScanLoadImage(file){return new Promise((resolve,reject)=>{const url=URL.createObjectURL(file),img=new Image();img.onload=()=>{URL.revokeObjectURL(url);resolve(img)};img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error("No se pudo abrir la imagen"))};img.src=url;});}
function pokemonSinglesScanCanvas(img,bottomOnly=false){
 const sourceX=0,sourceY=bottomOnly?Math.floor(img.naturalHeight*.55):0,sourceW=img.naturalWidth,sourceH=bottomOnly?Math.ceil(img.naturalHeight*.45):img.naturalHeight,maxWidth=bottomOnly?1800:1200,scale=Math.min(2.2,maxWidth/sourceW),canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(sourceW*scale));canvas.height=Math.max(1,Math.round(sourceH*scale));const ctx=canvas.getContext("2d",{willReadFrequently:true});ctx.drawImage(img,sourceX,sourceY,sourceW,sourceH,0,0,canvas.width,canvas.height);
 try{const data=ctx.getImageData(0,0,canvas.width,canvas.height),px=data.data;for(let i=0;i<px.length;i+=4){const gray=.299*px[i]+.587*px[i+1]+.114*px[i+2],v=Math.max(0,Math.min(255,(gray-128)*1.45+128));px[i]=px[i+1]=px[i+2]=v;}ctx.putImageData(data,0,0);}catch{}
 return canvas;
}
function pokemonSinglesScanExtract(text){
 const raw=String(text||"").replace(/[|\\]/g,"/").replace(/[‐‑–—]/g,"-").toUpperCase(),compact=raw.replace(/\s+/g," ");
 const fraction=compact.match(/\b(\d{1,3})\s*[\/／]\s*(\d{1,3})\b/);const setCandidates=[...compact.matchAll(/\b([A-Z]{1,4}\d[A-Z0-9]{0,3})\b/g)].map(m=>m[1]).filter(code=>!/^HP\d/.test(code));
 const prefixed=[...compact.matchAll(/\b([A-Z]{2,5})[\s-]?0*(\d{2,3})\b/g)].map(m=>({prefix:m[1],number:m[2]})).find(x=>!["HP","ILLUS","NO"].includes(x.prefix));
 if(fraction)return {query:`${Number(fraction[1])}/${Number(fraction[2])}`,number:String(Number(fraction[1])),denominator:String(Number(fraction[2])),setCode:setCandidates[0]||"",raw:compact};
 if(prefixed)return {query:String(Number(prefixed.number)),number:String(Number(prefixed.number)),denominator:"",setCode:prefixed.prefix,raw:compact};
 const numbers=[...compact.matchAll(/\b(\d{2,3})\b/g)].map(m=>m[1]).filter(n=>Number(n)>0&&Number(n)<1000);if(numbers.length)return {query:String(Number(numbers[numbers.length-1])),number:String(Number(numbers[numbers.length-1])),denominator:"",setCode:setCandidates[0]||"",raw:compact};
 return null;
}
async function pokemonSinglesScanFile(file){
 if(!file||pokemonSinglesScanBusy)return;pokemonSinglesScanBusy=true;const query=$("#pokemonSingleQuery"),home=$("#pokemonSinglesSearchHome"),status=$("#pokemonSingleSearchStatus");let previewUrl="";
 try{if(!String(file.type||"").startsWith("image/"))throw new Error("Selecciona una fotografía de la carta.");previewUrl=URL.createObjectURL(file);const preview=$("#pokemonSingleScanPreview");if(preview){preview.src=previewUrl;preview.hidden=false;}pokemonSinglesScanSetStatus("Preparando reconocimiento…","La primera vez puede descargar el motor OCR",4);const img=await pokemonSinglesScanLoadImage(file),worker=await pokemonSinglesScannerWorker();pokemonSinglesScanSetStatus("Identificando carta…","Leyendo la zona inferior de la carta",10);let result=await worker.recognize(pokemonSinglesScanCanvas(img,true)),hint=pokemonSinglesScanExtract(result?.data?.text||"");if(!hint){pokemonSinglesScanSetStatus("Identificando carta…","No veo el número abajo; revisando la carta completa",55);result=await worker.recognize(pokemonSinglesScanCanvas(img,false));hint=pokemonSinglesScanExtract(result?.data?.text||"");}
  if(!hint)throw new Error("No he podido leer el número de colección. Prueba con la carta recta, bien iluminada y ocupando casi toda la foto.");pokemonSinglesScanHint={...hint,at:Date.now()};if(query){clearTimeout(pokemonSinglesSearchTimer);query.value=hint.query;}if(home)home.hidden=true;pokemonSinglesScanSetStatus("Carta detectada",`${hint.query}${hint.setCode?` · código ${hint.setCode}`:""}. Buscando coincidencias…`,100);if(status)status.textContent=`Detectado ${hint.query}${hint.setCode?` · ${hint.setCode}`:""}. Buscando…`;await pokemonSinglesSearch(true);
 }catch(error){console.warn("Pokemon photo scan",error);pokemonSinglesScanHint=null;pokemonSinglesScanSetStatus("No he podido identificarla",error?.message||"Prueba con otra foto.",null);if(status)status.textContent="";}finally{pokemonSinglesScanBusy=false;setTimeout(()=>{if(previewUrl)URL.revokeObjectURL(previewUrl)},2500);}
}
function pokemonSinglesOpenScanDialog(){const dialog=$("#pokemonSingleScanDialog");if(!dialog)return;try{dialog.showModal();}catch{dialog.setAttribute("open","");}}
function pokemonSinglesWireScanner(){
 $("#pokemonSingleScan")?.addEventListener("click",pokemonSinglesOpenScanDialog);const dialog=$("#pokemonSingleScanDialog"),camera=$("#pokemonSingleCameraInput"),gallery=$("#pokemonSingleGalleryInput");$("#pokemonSingleScanClose")?.addEventListener("click",()=>dialog?.close?.());$("#pokemonSingleTakePhoto")?.addEventListener("click",()=>{dialog?.close?.();camera?.click();});$("#pokemonSingleChoosePhoto")?.addEventListener("click",()=>{dialog?.close?.();gallery?.click();});for(const input of [camera,gallery])input?.addEventListener("change",()=>{const file=input.files?.[0];input.value="";if(file)pokemonSinglesScanFile(file);});
}
function renderPokemonSinglesSearchView(){
 const root=$("#pokemonSinglesSearchView"),project=pokemonSinglesProject();if(!root||!project)return;pokemonSinglesPrimeCardmarketData();
 root.innerHTML=`<section class="pokemon-singles-shell pokemon-discover-shell"><header class="pokemon-singles-hero"><div><span>POKÉMON · BÚSQUEDA</span><h2>Buscar cartas</h2><p>Encuentra cualquier carta, consulta Cardmarket y añádela a su álbum y a Mis Singles.</p></div></header><section class="pokemon-discover-search"><div class="pokemon-discover-searchbox"><span>⌕</span><input id="pokemonSingleQuery" autocomplete="off" autocapitalize="off" spellcheck="false" inputmode="search" enterkeyhint="search" placeholder="Busca cartas, número o expansión…"><button id="pokemonSingleScan" class="pokemon-single-scan-button" type="button" aria-label="Identificar carta con una foto" title="Identificar con foto">📷</button><button id="pokemonSingleClear" type="button" aria-label="Limpiar">×</button></div><div class="pokemon-discover-filters"><select id="pokemonSingleLanguage" aria-label="Idioma"><option value="es">Castellano</option><option value="en">English</option><option value="ja">日本語</option><option value="zh-cn">中文</option></select><span class="pokemon-scan-language-note">La foto busca en el idioma seleccionado</span></div><div id="pokemonSingleSearchStatus" class="pokemon-single-search-status"></div><div id="pokemonSingleScanPanel" class="pokemon-single-scan-panel" hidden><img id="pokemonSingleScanPreview" alt="Foto de la carta" hidden><div><strong id="pokemonSingleScanTitle">Identificando carta…</strong><span id="pokemonSingleScanDetail"></span><i id="pokemonSingleScanProgress" hidden></i></div></div></section><div id="pokemonSinglesSearchHome">${pokemonSinglesRecentStrip("Añadidas recientemente",POKEMON_SINGLES_RECENT_ADDED_KEY,"added")}${pokemonSinglesRecentStrip("Vistas recientemente",POKEMON_SINGLES_RECENT_VIEWED_KEY,"viewed")}</div><div id="pokemonSingleSearchResults" class="pokemon-single-search-results pokemon-discover-results"></div><dialog id="pokemonSingleScanDialog" class="pokemon-single-scan-dialog"><button id="pokemonSingleScanClose" class="pokemon-single-scan-close" type="button" aria-label="Cerrar">×</button><div class="pokemon-single-scan-dialog-icon">📷</div><h3>Identificar carta</h3><p>Haz una foto nítida de la carta completa. StickerBase leerá el número de colección y abrirá el mismo buscador.</p><div class="pokemon-single-scan-tips"><span>✓ Carta recta</span><span>✓ Buena luz</span><span>✓ Número inferior visible</span></div><div class="pokemon-single-scan-dialog-actions"><button id="pokemonSingleTakePhoto" type="button">📷 Hacer foto</button><button id="pokemonSingleChoosePhoto" type="button">▧ Elegir de Fotos</button></div><small>Especialmente útil para cartas japonesas o chinas: selecciona su idioma antes de escanear.</small></dialog><input id="pokemonSingleCameraInput" type="file" accept="image/*" capture="environment" hidden><input id="pokemonSingleGalleryInput" type="file" accept="image/*" hidden></section>`;
 const query=$("#pokemonSingleQuery"),home=$("#pokemonSinglesSearchHome");pokemonSinglesWireScanner();query?.addEventListener("input",()=>{clearTimeout(pokemonSinglesSearchTimer);const value=query.value.trim();if(home)home.hidden=value.length>=2;if(!pokemonSinglesScanBusy&&pokemonSinglesScanHint&&value!==pokemonSinglesScanHint.query)pokemonSinglesScanHint=null;if(value.length<2){pokemonSinglesSearchResults=[];$("#pokemonSingleSearchStatus").textContent="";renderPokemonSinglesSearchResults();return;}pokemonSinglesSearchTimer=setTimeout(()=>pokemonSinglesSearch(false),300);});query?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();pokemonSinglesSearch(true)}});$("#pokemonSingleClear")?.addEventListener("click",()=>{query.value="";pokemonSinglesScanHint=null;pokemonSinglesSearchResults=[];if(home)home.hidden=false;const panel=$("#pokemonSingleScanPanel");if(panel)panel.hidden=true;renderPokemonSinglesSearchResults();query.focus();});$("#pokemonSingleLanguage")?.addEventListener("change",()=>{if((query?.value||"").trim().length>=2)pokemonSinglesSearch(true);});document.querySelectorAll("[data-recent-kind]").forEach(el=>el.onclick=()=>{const key=el.dataset.recentKind==="added"?POKEMON_SINGLES_RECENT_ADDED_KEY:POKEMON_SINGLES_RECENT_VIEWED_KEY;const card=pokemonSinglesRecentRead(key)[Number(el.dataset.recentIndex)];if(card?.image)openPokemonCardImageViewer(card.image,card.number,{name:card.name,images:{large:card.imageLarge||""}});});renderPokemonSinglesSearchResults();
}
function pokemonSinglesEuro(value){const n=Number(value);return Number.isFinite(n)&&n>=0?new Intl.NumberFormat("es-ES",{style:"currency",currency:"EUR",minimumFractionDigits:2,maximumFractionDigits:2}).format(n):""}
function pokemonSinglesCardmarketView(card){
 const cm=card?.cardmarket||card?.pricing?.cardmarket;if(!cm)return null;const has=value=>Number.isFinite(Number(value))&&Number(value)>0;
 const variants=card?.variants||{},preferHolo=variants.normal===false&&(variants.holo===true||variants.reverse===true);
 const regular={trend:cm.trend,avg7:cm.avg7,avg30:cm.avg30,low:cm.low,avg:cm.avg},holo={trend:cm["trend-holo"],avg7:cm["avg7-holo"],avg30:cm["avg30-holo"],low:cm["low-holo"],avg:cm["avg-holo"]};
 const usable=obj=>[obj.trend,obj.avg7,obj.avg30,obj.avg,obj.low].some(has);let chosen=preferHolo&&usable(holo)?holo:(usable(regular)?regular:(usable(holo)?holo:null));if(!chosen)return null;
 const primary=[chosen.trend,chosen.avg7,chosen.avg30,chosen.avg,chosen.low].find(has);if(!has(primary))return null;return {primary:Number(primary),avg7:has(chosen.avg7)?Number(chosen.avg7):null,avg30:has(chosen.avg30)?Number(chosen.avg30):null,low:has(chosen.low)?Number(chosen.low):null,holo:chosen===holo,updated:cm.updated||""};
}
function pokemonSinglesCardmarketHtml(card,compact=false){const price=pokemonSinglesCardmarketView(card);if(!price)return "";const details=[];if(price.avg7!==null)details.push(`7d ${pokemonSinglesEuro(price.avg7)}`);if(price.avg30!==null)details.push(`30d ${pokemonSinglesEuro(price.avg30)}`);return `<div class="pokemon-cardmarket-price ${compact?"is-compact":""}"><span>Cardmarket</span><strong>${pokemonSinglesEuro(price.primary)}</strong>${details.length?`<small>${details.join(" · ")}</small>`:""}</div>`}
function pokemonSingleOwnedCardHtml(card){
 const incoming=card.status==="incoming";return `<article class="pokemon-single-owned ${incoming?"is-incoming":""}" data-single-id="${collectionSafeText(card.id)}"><div class="pokemon-single-owned-image ${card.image?"is-clickable":""}" ${card.image?`data-owned-single-preview="${collectionSafeText(card.id)}" role="button" tabindex="0" aria-label="Ver ${collectionSafeText(card.name||"carta Pokémon")} en grande"`:""}>${card.image?`<img src="${collectionSafeText(card.image)}" alt="${collectionSafeText(card.name||"Carta Pokémon")}" loading="lazy" onerror="pokemonSinglesOwnedImageError(this)">`:'<span>PK</span>'}</div><div class="pokemon-single-owned-copy"><small>${collectionSafeText(card.setName||"Pokémon TCG")}</small><strong>${collectionSafeText(card.name||"Carta Pokémon")}</strong><span>#${collectionSafeText(card.number||"—")}${card.rarity?` · ${collectionSafeText(card.rarity)}`:""}</span><label class="pokemon-single-language-edit"><span>Idioma</span><select data-single-language="${collectionSafeText(card.id)}" aria-label="Idioma de ${collectionSafeText(card.name||"carta Pokémon")}"><option value="es" ${card.language==="es"?"selected":""}>Castellano</option><option value="en" ${card.language==="en"?"selected":""}>English</option><option value="ja" ${card.language==="ja"?"selected":""}>日本語</option><option value="zh-cn" ${card.language==="zh-cn"?"selected":""}>中文</option></select></label>${pokemonSinglesCardmarketHtml(card,true)}</div><div class="pokemon-single-owned-actions"><button type="button" data-single-incoming="${collectionSafeText(card.id)}" class="${incoming?"active":""}">${incoming?"📦 En camino":"＋ En camino"}</button><button type="button" data-single-remove="${collectionSafeText(card.id)}">Eliminar</button></div></article>`;
}
async function pokemonSinglesFetchJson(url){
 const fetchOne=async target=>{const controller=typeof AbortController!=="undefined"?new AbortController():null,timer=controller?setTimeout(()=>controller.abort(),6500):null;try{const response=await fetch(target,{cache:"no-store",signal:controller?.signal});if(!response.ok)throw new Error(`HTTP ${response.status}`);return await response.json();}finally{if(timer)clearTimeout(timer);}};
 const raw=String(url||"");
 if(raw.startsWith(POKEMON_TCGDEX_API)){const suffix=raw.slice(POKEMON_TCGDEX_API.length),targets=[POKEMON_TCGDEX_API+suffix,POKEMON_TCGDEX_BETA_API+suffix];let last=null;for(const target of targets){try{return await fetchOne(target)}catch(error){last=error;console.warn("TCGdex endpoint failed",target,error?.message||error);}}throw last||new Error("TCGdex unavailable");}
 return fetchOne(raw);
}

function pokemonSinglesLocalSearch(raw,lang="es"){
 const q=normalizeTradeName(raw),fraction=String(raw||"").match(/(\d{1,3})\s*\/\s*(\d{1,3})/),onlyNumber=String(raw||"").match(/^\s*(\d{1,3})\s*$/),trailing=String(raw||"").match(/^(.*?)\s+(\d{1,3})\s*$/),wantedNumber=String(Number((fraction||onlyNumber||trailing||[])[1]||trailing?.[2]||0)||"");
 const rows=[];Object.values(projects||{}).forEach(project=>{const type=inferCollectionType(project);if(!isPokemonCollectionType(type)||type==="pokemon-singles")return;const def=POKEMON_SET_DEFS[type]||{},meta=project?.pokemonMeta||{};Object.entries(meta).forEach(([code,item])=>{const number=String(item?.number||code||""),name=String(item?.name||`Carta ${number}`),hay=normalizeTradeName(`${name} ${project.name||""} ${number}`);if(q&&!hay.includes(q)&&!(wantedNumber&&String(Number(number.replace(/\D/g,"")||0))===wantedNumber))return;const setId=def?.setId||"",tcgSet=pokemonSinglesTcgdexSetCandidates(setId)[1]||setId;rows.push({id:`local:${project.id}:${code}`,tcgdexId:tcgSet?`${tcgSet}-${number}`:"",name,number,setName:project.name||"",setId,rarity:item?.rarity||"",language:lang,image:item?.images?.small||pokemonDirectImageUrl(type,code,"small")||"",variants:item?.variants||{},cardmarket:null,source:"local"});});});return rows.slice(0,48);
}
async function pokemonSinglesPokemonTcgSearch(raw,lang="en"){
 try{const clean=String(raw||"").trim(),fraction=clean.match(/(\d{1,3})\s*\/\s*(\d{1,3})/),onlyNumber=clean.match(/^\s*(\d{1,3})\s*$/),trailing=clean.match(/^(.*?)\s+(\d{1,3})\s*$/);let q="";if(fraction||onlyNumber){q=`number:${Number((fraction||onlyNumber)[1])}`;}else{const name=(trailing?.[1]||clean).replace(/["\\]/g," ").trim();if(!name)return [];q=`name:${name.split(/\s+/).map(x=>`${x}*`).join(" ")}`;if(trailing?.[2])q+=` number:${Number(trailing[2])}`;}const payload=await pokemonSinglesFetchJson(`${POKEMON_TCG_API}/cards?q=${encodeURIComponent(q)}&pageSize=50&orderBy=-set.releaseDate`),cards=Array.isArray(payload?.data)?payload.data:[];return cards.map(card=>({id:`ptcg:${card.id}`,tcgdexId:"",name:card.name||"Carta Pokémon",englishName:card.name||"",number:String(card.number||""),setName:card.set?.name||"",setId:card.set?.id||"",setCardCount:String(card.set?.printedTotal||card.set?.total||""),rarity:card.rarity||"",language:lang,image:card.images?.small||card.images?.large||"",imageLarge:card.images?.large||card.images?.small||"",variants:{},cardmarket:pokemonSinglesNormalizeCardmarket(card.cardmarket||card.cardmarket?.prices||null),source:"ptcg"}));}catch(error){console.warn("Pokemon TCG fallback search",error);return []}
}
function pokemonSinglesMergeSearchRows(...groups){const out=[],seen=new Set();for(const group of groups){for(const card of group||[]){const key=[normalizeTradeName(card?.name||""),normalizeTradeName(card?.setName||""),pokemonSinglesCollectorKey(card?.number||"")].join("|");if(seen.has(key))continue;seen.add(key);out.push(card);}}return out;}

async function pokemonSinglesSearch(force=false){
 if(pokemonSinglesSearchBusy){pokemonSinglesSearchQueued=true;return;}const input=$("#pokemonSingleQuery"),lang=$("#pokemonSingleLanguage")?.value||"es",raw=input?.value?.trim()||"",status=$("#pokemonSingleSearchStatus");if(raw.length<2){if(status)status.textContent="Escribe al menos 2 caracteres.";return;}
 pokemonSinglesSearchBusy=true;pokemonSinglesSearchPriceHydrationToken++;if(status)status.textContent="Buscando cartas…";const button=$("#pokemonSingleSearchButton");if(button)button.disabled=true;
 try{
   const local=pokemonSinglesLocalSearch(raw,lang);let tcgRows=[],tcgFailed=false;
   try{const fraction=raw.match(/(\d{1,3})\s*\/\s*(\d{1,3})/),onlyNumber=raw.match(/^\s*(\d{1,3})\s*$/),trailing=raw.match(/^(.*?)\s+(\d{1,3})\s*$/);let briefs=[];if(fraction||onlyNumber){const localId=String(Number((fraction||onlyNumber)[1]));briefs=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/${lang}/cards?localId=${encodeURIComponent(localId)}`);}else{const nameQuery=(trailing?.[1]||raw).trim();briefs=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/${lang}/cards?name=${encodeURIComponent(nameQuery)}`);if(trailing?.[2]&&Array.isArray(briefs)){const wanted=Number(trailing[2]);briefs=briefs.filter(card=>Number(card.localId)===wanted||Number(String(card.localId||'').replace(/\D/g,''))===wanted);}}if(!Array.isArray(briefs))briefs=[];briefs=briefs.slice(0,24);const details=(await Promise.all(briefs.map(async card=>{try{return await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/${lang}/cards/${encodeURIComponent(card.id)}`)}catch{return card}}))).filter(Boolean);tcgRows=await Promise.all(details.map(async card=>{let image=pokemonSinglesImageUrl(card.image,"low"),englishName="";if(lang!=="en"){try{const enCard=await pokemonSinglesFetchJson(`${POKEMON_TCGDEX_API}/en/cards/${encodeURIComponent(card.id)}`);if(!image)image=pokemonSinglesImageUrl(enCard?.image,"low");englishName=enCard?.name||"";}catch{}}else englishName=card.name||"";return {id:`${lang}:${card.id}`,tcgdexId:card.id,name:card.name||"Carta Pokémon",englishName,number:String(card.localId||""),setName:card.set?.name||"",setId:card.set?.id||"",setCardCount:String(card.set?.cardCount?.official||card.set?.cardCount?.total||""),rarity:card.rarity||"",language:lang,image,variants:card.variants||{},cardmarket:pokemonSinglesNormalizeCardmarket(card.pricing?.cardmarket||null),source:"tcgdex"};}));}catch(error){tcgFailed=true;console.warn("TCGdex search unavailable",error);}
   let external=[];if(tcgFailed||!tcgRows.length){external=await pokemonSinglesPokemonTcgSearch(raw,"en");}
   pokemonSinglesSearchResults=pokemonSinglesMergeSearchRows(local,tcgRows,external).slice(0,48);
   pokemonSinglesSearchResults.forEach(result=>{if(!result.image)result.image=pokemonSinglesFallbackImage(result);});
   const nq=normalizeTradeName(raw);pokemonSinglesSearchResults.sort((a,b)=>{const score=c=>{const n=normalizeTradeName(c.name||""),set=normalizeTradeName(c.setName||""),num=String(c.number||"");let v=0;if(n===nq)v+=100;if(n.startsWith(nq))v+=60;if(n.includes(nq))v+=35;if(nq.includes(n))v+=15;if(set.includes(nq))v+=20;if(num&&nq.includes(num))v+=45;if(c.source==="local")v+=10;if(pokemonSinglesScanHint){const hint=pokemonSinglesScanHint;if(hint.number&&String(Number(String(c.number||"").replace(/\D/g,"")||0))===String(Number(hint.number)))v+=90;if(hint.denominator&&String(Number(c.setCardCount||0))===String(Number(hint.denominator)))v+=110;if(hint.setCode){const code=normalizeTradeName(hint.setCode),id=normalizeTradeName(c.setId||"");if(code&&id&&(id===code||id.includes(code)||code.includes(id)))v+=120;}}for(const token of nq.split(/\s+/).filter(Boolean)){if(n.includes(token))v+=8;if(set.includes(token))v+=5;if(num===token)v+=15;}return v};return score(b)-score(a)});
   if(status){const source=tcgFailed?" · modo de respaldo":"";status.textContent=pokemonSinglesSearchResults.length?`${pokemonSinglesSearchResults.length} coincidencias${source}`:`No he encontrado cartas para “${raw}”.${tcgFailed?" El catálogo principal no responde.":""}`;}renderPokemonSinglesSearchResults();
   // Los precios nunca bloquean la búsqueda. Primero usamos el snapshot local; si no existe,
   // Cardmarket se actualiza en segundo plano y esta vista se refresca al terminar.
   void pokemonSinglesHydrateVisibleSearchPrices();
   const priceToken=pokemonSinglesSearchPriceHydrationToken;Promise.all(pokemonSinglesSearchResults.map(async result=>{if(result.cardmarket&&result.image)return false;const fallback=await pokemonSinglesPokemonTcgFallback(result);if(priceToken!==pokemonSinglesSearchPriceHydrationToken)return false;let changed=false;if(fallback){if(!result.cardmarket&&fallback.cardmarket){result.cardmarket=fallback.cardmarket;changed=true;}if(!result.image&&fallback.image){result.image=fallback.image;result.imageLarge=fallback.imageLarge||fallback.image;changed=true;}}return changed;})).then(changes=>{if(priceToken===pokemonSinglesSearchPriceHydrationToken&&changes.some(Boolean))renderPokemonSinglesSearchResults();}).catch(()=>{});
 }catch(error){console.warn("Pokemon singles search",error);if(status)status.textContent="No se pudo consultar ningún catálogo. Inténtalo de nuevo en unos segundos.";pokemonSinglesSearchResults=[];renderPokemonSinglesSearchResults();}
 finally{pokemonSinglesSearchBusy=false;if(button)button.disabled=false;if(pokemonSinglesSearchQueued){pokemonSinglesSearchQueued=false;setTimeout(()=>pokemonSinglesSearch(false),0);}}
}

function pokemonSinglesTargetState(card,target){
 if(!target){
  const single=pokemonSinglesProject()?.pokemonSingles?.find(item=>item.id===card.id);
  if(!single)return {owned:false,incoming:false,label:""};
  return {owned:single.status!=="incoming",incoming:single.status==="incoming",label:single.status==="incoming"?"📦 Ya la tienes en camino":"✓ Ya la tienes"};
 }
 const {project,section,code,options}=target;
 const variant=pokemonSinglesRouteVariant(card,target);
 let owned=false;
 if(options.length){owned=(Number(project?.pokemonVariants?.[code]?.[variant])||0)>0;}
 else owned=(Number(project?.inventory?.[section]?.[code])||0)>0;
 const incoming=pokemonIncomingHas(project,section,code,variant);
 return {owned,incoming,label:incoming?`📦 Ya está en camino · ${project.name}`:(owned?`✓ Ya la tienes · ${project.name}`:"")};
}
function pokemonSinglesRefreshResultState(article,index){
 const card=pokemonSinglesSearchResults[index];if(!card||!article)return;const target=pokemonSinglesCollectionTarget(card),state=pokemonSinglesTargetState(card,target);
 const badge=article.querySelector("[data-single-existing-state]");if(badge){badge.textContent=state.label;badge.hidden=!state.label;badge.classList.toggle("is-incoming",state.incoming);}
 const own=article.querySelector('[data-single-status="owned"]'),incoming=article.querySelector('[data-single-status="incoming"]');
 if(own){own.disabled=state.owned;own.textContent=state.owned?"✓ Ya la tienes":"＋ La tengo";}
 if(incoming){incoming.disabled=state.incoming;incoming.textContent=state.incoming?"📦 Ya en camino":"📦 En camino";}
}
function renderPokemonSinglesSearchResults(){
 const root=$("#pokemonSingleSearchResults");if(!root)return;if(!pokemonSinglesSearchResults.length){root.innerHTML="";return;}
 const cardHtml=(card,index,top=false)=>{const target=pokemonSinglesCollectionTarget(card),canonicalImage=target?(target.meta?.images?.small||pokemonDirectImageUrl(target.type,target.code,"small")||""):"",displayImage=canonicalImage||card.image||pokemonSinglesFallbackImage(card),variant=target?.options?.length?`<select class="pokemon-single-route-variant" data-single-variant="${collectionSafeText(card.id)}">${target.options.map(([key,label])=>`<option value="${collectionSafeText(key)}">${collectionSafeText(label)}</option>`).join("")}</select>`:"",destination=target?`<em class="pokemon-single-destination">→ ${collectionSafeText(target.project.name)}</em>`:`<em class="pokemon-single-destination">→ Mis Singles</em>`;card.displayImage=displayImage;return `<article class="pokemon-discover-card ${top?"is-top":""}" data-single-result-index="${index}"><div class="pokemon-discover-card-image ${displayImage?"is-clickable":""}" ${displayImage?`data-single-preview="${index}" role="button" tabindex="0"`:""}>${displayImage?`<img src="${collectionSafeText(displayImage)}" alt="${collectionSafeText(card.name)}" loading="lazy" onerror="this.style.display='none';this.parentElement.classList.add('image-missing');">`:'<span>PK</span>'}</div><div class="pokemon-discover-card-copy"><small>${collectionSafeText(card.setName)}</small><strong>${collectionSafeText(card.name)}</strong><span>#${collectionSafeText(card.number)}${card.rarity?` · ${collectionSafeText(card.rarity)}`:""}</span>${pokemonSinglesCardmarketHtml(card,!top)}${destination}${variant}<em class="pokemon-single-existing-state" data-single-existing-state hidden></em><div class="pokemon-discover-actions"><button type="button" data-add-single="${index}" data-single-status="owned">＋ La tengo</button><button type="button" data-add-single="${index}" data-single-status="incoming">📦 En camino</button></div></div></article>`};
 const top=pokemonSinglesSearchResults[0],rest=pokemonSinglesSearchResults.slice(1);root.innerHTML=`<section class="pokemon-discover-top"><div class="pokemon-discover-section-head"><span>Mejor resultado</span></div>${cardHtml(top,0,true)}</section>${rest.length?`<section class="pokemon-discover-more"><div class="pokemon-discover-section-head"><span>Cartas y versiones</span><small>${pokemonSinglesSearchResults.length} coincidencias</small></div><div class="pokemon-discover-grid">${rest.map((c,i)=>cardHtml(c,i+1,false)).join("")}</div></section>`:""}`;
 root.querySelectorAll("[data-add-single]").forEach(button=>button.onclick=()=>pokemonAddSingle(Number(button.dataset.addSingle),button.dataset.singleStatus||"owned"));root.querySelectorAll("[data-single-result-index]").forEach(article=>pokemonSinglesRefreshResultState(article,Number(article.dataset.singleResultIndex)));root.querySelectorAll("[data-single-variant]").forEach(select=>select.addEventListener("change",()=>{const article=select.closest("[data-single-result-index]");if(article)pokemonSinglesRefreshResultState(article,Number(article.dataset.singleResultIndex));}));root.querySelectorAll("[data-single-preview]").forEach(el=>{const open=()=>{const card=pokemonSinglesSearchResults[Number(el.dataset.singlePreview)];if(card?.displayImage||card?.image){pokemonSinglesRecentPush(POKEMON_SINGLES_RECENT_VIEWED_KEY,card);openPokemonCardImageViewer(card.displayImage||card.image,card.number,{name:card.name});}};el.onclick=open;el.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();open();}};});
}
function pokemonAddSingle(index,status="owned"){
 const project=pokemonSinglesProject(),card=pokemonSinglesSearchResults[index];if(!project||!card)return;const target=pokemonSinglesCollectionTarget(card);
 if(target){const state=pokemonSinglesTargetState(card,target),variant=pokemonSinglesRouteVariant(card,target);
  if(status==="owned"&&state.owned){const mirror=pokemonSinglesUpsertMirror({...card,routeVariant:variant},target,"owned",{manualKeep:true,variant,cardmarketFetchedAt:card.cardmarket?new Date().toISOString():""});if(mirror&&card.cardmarket)mirror.cardmarket=card.cardmarket;persistProjects();scheduleCloudSave();renderPokemonSinglesSearchResults();showToast(`✓ Ya la tienes en ${target.project.name}; ficha guardada en Mis Singles`);return;}
  if(status==="incoming"&&state.incoming){pokemonSinglesUpsertMirror({...card,routeVariant:variant},target,"incoming",{manualKeep:true,variant,cardmarketFetchedAt:card.cardmarket?new Date().toISOString():""});persistProjects();scheduleCloudSave();renderPokemonSinglesSearchResults();showToast(`📦 Ya está En camino en ${target.project.name} y visible en Mis Singles`);return;}
  pokemonSinglesRecentPush(POKEMON_SINGLES_RECENT_ADDED_KEY,{...card,displayImage:card.displayImage||card.image});pokemonSinglesAddToCollection(card,status,target);return;
 }
 project.pokemonSingles=project.pokemonSingles||[];const existing=project.pokemonSingles.find(item=>item.id===card.id);if(existing){showToast(existing.status==="incoming"?"📦 Esta carta ya está En camino":"✓ Esta carta ya está en Mis Singles");return;}project.pokemonSingles.push({...card,status:status==="incoming"?"incoming":"owned",manualKeep:true,addedAt:new Date().toISOString(),cardmarketFetchedAt:card.cardmarket?new Date().toISOString():""});pokemonSinglesRecentPush(POKEMON_SINGLES_RECENT_ADDED_KEY,{...card,displayImage:card.displayImage||card.image});persistProjects();scheduleCloudSave();if(mainTab==="trade")renderPokemonSinglesSearchResults();updateNavigationBadges();showToast(status==="incoming"?"📦 Single añadido a En camino":"✓ Single añadido");
}
function wirePokemonSingleCards(){
 const project=pokemonSinglesProject();if(!project)return;
 document.querySelectorAll("[data-owned-single-preview]").forEach(el=>{const open=()=>{const card=project.pokemonSingles.find(item=>item.id===el.dataset.ownedSinglePreview);if(card?.image)openPokemonCardImageViewer(card.image,card.number,{name:card.name,images:{large:card.imageLarge||""}});};el.onclick=open;el.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();open();}};});
 ...
 document.querySelectorAll("[data-single-language]").forEach(select=>select.onchange=()=>{const card=project.pokemonSingles.find(item=>item.id===select.dataset.singleLanguage);if(card)pokemonSinglesChangeOwnedLanguage(card,select.value);});
 document.querySelectorAll("[data-single-remove]").forEach(button=>button.onclick=()=>{const card=project.pokemonSingles.find(item=>item.id===button.dataset.singleRemove);if(!card)return;const target=pokemonSinglesTargetFromMirror(card)||pokemonSinglesCollectionTarget(card),message=target?`¿Eliminar ${card.name} de Mis Singles, ${target.project.name} y En camino?`:`¿Eliminar ${card.name} de Mis Singles?`;if(!confirm(message))return;pokemonSinglesDeleteEverywhere(card);persistProjects();scheduleCloudSave();renderPokemonSinglesCollection();updateNavigationBadges();showToast(target?"Carta eliminada de todos los apartados":"Single eliminado");});
}
...
function setMainTab(tab){
 ...
 const pokemonIncomingView=$("#pokemonIncomingView");
 const pokemonSinglesSearchView=$("#pokemonSinglesSearchView");
 const collectionsView=$("#collectionsView");
 const missingView=$("#missingView");
 const activeType=inferCollectionType(projects?.[activeProjectId]);
 const pokemonActive=activeType==="pokemon-singles"||isPokemonCollectionType(activeType)||document.body.classList.contains("pokemon-ui-active"),singlesActive=activeType==="pokemon-singles";
 ...
 if(appScroll){
   ...
   if(tab==="collection"){
     requestAnimationFrame(()=>appScroll.scrollTo({top:0,left:0,behavior:"auto"}));
     setTimeout(()=>appScroll.scrollTo({top:0,left:0,behavior:"auto"}),60);
   }
 }
}

function renderAll(){
 applyCollectionIdentity();renderPokemonIncomingView();
...
teamSearch.oninput=()=>{
 ...
 if(isPokemonCollectionType(searchType)){
   suggestions.hidden=true;syncMainSearchSpace();searchType==="pokemon-singles"?renderPokemonSinglesCollection():renderPokemonCollection();return;
 }
...
function collectionLibraryGroups(){
 return [
  {type:"world-cup-2026",label:"World Cup 2026",short:"26",theme:"worldcup",match:p=>inferCollectionType(p)==="world-cup-2026"},
  {type:"liga-este-2026-27",label:"Liga Este 2026/27",short:"LE",theme:"ligaeste",match:p=>inferCollectionType(p)==="liga-este-2026-27"},
  {type:"megacracks-2026-27",label:"Megacracks 2026/27",short:"MC",theme:"megacracks",match:p=>inferCollectionType(p)==="megacracks-2026-27"},
  {type:"pokemon",label:"Pokémon TCG",short:"PK",theme:"pokemon",match:p=>isPokemonCollectionType(inferCollectionType(p))}
 ];
}
...
function renderCollections(){
 ...
 const pokemon=items.filter(p=>isPokemonCollectionType(inferCollectionType(p))&&inferCollectionType(p)!=="pokemon-singles");
 const singlesProject=items.find(p=>inferCollectionType(p)==="pokemon-singles")||null;
 const chase={"pokemon-pitch-black":116,"pokemon-chaos-rising":116,"pokemon-perfect-order":121,"pokemon-surging-sparks":219,"pokemon-151":205,"pokemon-phantasmal-flames":125,"pokemon-ascended-heroes":271,"pokemon-first-partner":37,"pokemon-mega-evolution":179,"pokemon-destined-rivals":231};
 ...
 const renderCard=(p)=>{
   ...
   const image=isPokemon&&ptype!=="pokemon-singles"?pokemonDirectImageUrl(ptype,chase[ptype]||1,"small"):(footballCover[ptype]||"");
   const singlesCount=ptype==="pokemon-singles"?(p.pokemonSingles||[]).length:0;
   const subtitle=ptype==="pokemon-singles"?`${singlesCount} cartas · colección libre`:(isPokemon?`${s.progress}% · ${s.different}/${pokemonTotal}`:`${p.target} ${albumWord(p.target)} · ${s.progress}%`);
 ...
 const singlesCard=singlesProject?renderCard(singlesProject):`<article class="collection-gallery-card collection-gallery-pokemon pokemon-library-singles is-virtual"><button type="button" class="collection-gallery-open" data-open-pokemon-singles aria-label="Abrir Mis Singles"><span class="collection-gallery-cover pokemon-singles-cover"><span>★</span></span><span class="collection-gallery-name">Mis Singles</span><span class="collection-gallery-meta">Cartas sueltas de cualquier expansión</span></button></article>`;
 if(pokemon.length||singlesProject||true)sections.push(renderFolder("pokemon","Pokémon TCG",pokemon.length+1,`<div class="collection-gallery-grid collection-gallery-grid-pokemon">${pokemon.map(renderCard).join('')}${singlesCard}</div>`,1));
 ...
 list.querySelector("[data-open-pokemon-singles]")?.addEventListener("click",openPokemonSinglesCollection);
