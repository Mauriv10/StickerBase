...
let tradeAnalyzerGiveSelection=new Set();
...
function renderTradeDetectedItems(items,selectable=false,selectedKeys=new Set()){
 ...
 return `<div class="trade-detected-list">${items.map(item=>{
  const name=type==="world-cup-2026"?`${item.team} ${item.displayCode}`:clubTradeNameForItem(type,item),key=tradeStickerKey(item),safe=item.available>0&&!isTradeStar(item)&&!isTradeProtected(item),selected=selectedKeys.has(key);
  ...
  return `<article class="trade-detected-item${selected?" selected":""}"><div class="trade-detected-icon">${flagHTML(item.team)}</div><div class="trade-detected-copy"><strong>${escapeTradeHtml(name)}</strong><small>${escapeTradeHtml(item.team)} · ${escapeTradeHtml(item.displayCode)}</small>${status}</div><div class="trade-detected-owned">x${item.owned}</div>${selectable&&safe?`<button type="button" class="trade-use-choice${selected?" selected":""}" data-trade-use="${escapeTradeHtml(key)}">${selected?"Usar ✓":"Elegir"}</button>`:""}</article>`;
 }).join("")}</div>`;
}
function renderTradeAnalyzerResult(){
 ...
 const safeKeys=new Set(safeAvailable.map(tradeStickerKey));for(const key of [...tradeAnalyzerGiveSelection])if(!safeKeys.has(key))tradeAnalyzerGiveSelection.delete(key);
 for(const rows of sourceGroups.values()){const safeRows=rows.filter(x=>safeKeys.has(tradeStickerKey(x)));if(safeRows.length===1)tradeAnalyzerGiveSelection.add(tradeStickerKey(safeRows[0]));}
 const selected=safeAvailable.filter(item=>tradeAnalyzerGiveSelection.has(tradeStickerKey(item))),previewItems=selected.map(item=>({...item,selectedUnits:item.available})),previewUnits=previewItems.reduce((sum,item)=>sum+(item.selectedUnits||0),0),withoutStock=analysed.filter(x=>x.available<=0).length,ambiguousAvailable=[...sourceGroups.values()].some(rows=>rows.filter(x=>safeKeys.has(tradeStickerKey(x))).length>1);
 ...
 result.querySelectorAll("[data-trade-use]").forEach(button=>button.addEventListener("click",()=>{const key=button.dataset.tradeUse;if(tradeAnalyzerGiveSelection.has(key))tradeAnalyzerGiveSelection.delete(key);else tradeAnalyzerGiveSelection.add(key);renderTradeAnalyzerResult();}));
...
function openTradeAnalyzer(){
 tradeAnalyzerGiveSelection=new Set();tradeReceiveSelection=new Map();
